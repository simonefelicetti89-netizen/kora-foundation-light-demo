...
export default function MyKoraHome() {
  ...
  return (
    <div className="space-y-6" data-testid="my-kora-home">
      ...
      {/* ── 5b. PERSONAL IMPACT BALANCE — card di ingresso ──────────────────── */}
      <div
        className="rounded-xl border border-[rgba(181,81,46,0.20)] bg-[rgba(181,81,46,0.04)] p-5 flex flex-col gap-3"
        data-testid="pib-entry-card"
      >
        <div>
          <h3 className="text-sm font-bold text-[rgba(6,3,43,0.90)]" style={{ fontFamily: 'Plus Jakarta Sans, var(--font-jakarta), system-ui, sans-serif' }}>
            Personal Impact Balance
          </h3>
          <p className="text-xs text-[rgba(6,3,43,0.48)] mt-1" style={{ fontFamily: 'Plus Jakarta Sans, var(--font-jakarta), system-ui, sans-serif' }}>
            Vedi il tuo bilancio privato, la tua Activation Signature e il mix dei pillar del periodo.
          </p>
        </div>
        <Link
          href="/my-kora/personal-impact-balance"
          className="inline-flex items-center gap-1 rounded-lg bg-[#06032B] px-4 py-2 text-xs font-bold text-white hover:opacity-90 transition-opacity self-start"
          style={{ fontFamily: 'Plus Jakarta Sans, var(--font-jakarta), system-ui, sans-serif', textDecoration: 'none' }}
          data-testid="pib-entry-cta"
        >
          Apri il Personal Impact Balance →
        </Link>
      </div>
