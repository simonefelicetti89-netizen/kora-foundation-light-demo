...
export default function CompanySetupPage() {
  ...
  function handleHeadcount(raw: string) {
    ...
    <div className="space-y-8 max-w-3xl">
      ...
      <div>
        <p className="text-xs font-semibold uppercase tracking-widest text-slate-400">
          KORA Foundation Light / Company Setup
        </p>
        <h1 className="text-xl font-bold text-slate-900 mt-0.5">Configura Nuova Azienda</h1>
        <p className="text-sm text-slate-500 mt-1">
          Registra il profilo aziendale per avviare il pipeline KORA.
        </p>
      </div>
      ...
      <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-xs text-amber-800 leading-relaxed">
        ...
        Questa azienda è una bozza demo di sessione. La persistenza database sarà collegata nella versione production.
      ...
      <section className="space-y-4">
        <div>
          <h2 className="text-sm font-semibold text-slate-800">A — Identità Aziendale</h2>
          <p className="text-xs text-slate-400 mt-0.5">Nome, ragione sociale e informazioni di base.</p>
        </div>
      ...
      <section className="space-y-4 border-t border-slate-100 pt-6">
        <div>
          <h2 className="text-sm font-semibold text-slate-800">B — Workforce</h2>
          <p className="text-xs text-slate-400 mt-0.5">
            Dimensione e struttura. Foundation Light richiede almeno 30 lavoratori.
          </p>
        </div>
        ...
        <div className="rounded border border-blue-100 bg-blue-50 px-3 py-2 text-[10px] text-blue-700 leading-relaxed">
          KORA misura l&apos;organizzazione, non gli individui. Nessun dato individuale è visibile ai ruoli aziendali.
          Cluster &lt; 10 lavoratori vengono soppressi per privacy.
        </div>
      </section>
      ...
      <section className="space-y-4 border-t border-slate-100 pt-6">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            Seleziona un template o lascia che KORA ne suggerisca uno in base al profilo.
          </p>
        </div>
            ...
            return (
              <button
                ...
                onClick={() =>
                  handleChange(
                    ...
                    isSelected ? undefined : tmpl.template_id,
                  )
                }
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  ...
                  {isSuggested && (
                    <span className="rounded border border-emerald-300 bg-white px-1.5 py-0.5 text-[8px] font-semibold text-emerald-700 shrink-0">
                      SUGGERITO
                    </span>
                  )}
                  {isSelected && (
                ...
                <div className="mt-2 flex flex-wrap gap-1">
                  {tmpl.suggested_pillars.map((p) => (
                    <span
                      key={p}
                      className={`rounded px-1 py-0.5 text-[8px] font-mono font-semibold ${
                ...
                <p className={`text-[9px] mt-1.5 italic ${isSelected ? 'text-slate-400' : 'text-slate-400'}`}>
                  {tmpl.activation_benchmark_note}
                </p>
              </button>
            );
          })}
      ...
      <section className="space-y-4 border-t border-slate-100 pt-6">
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          <div className="space-y-1">
            <label className="text-xs font-semibold text-slate-600">
              Referente principale <span className="text-rose-500">*</span>
            </label>
            <input
              type="text"
      ...
      <section className="space-y-2 border-t border-slate-100 pt-6">
        <h2 className="text-sm font-semibold text-slate-800">E — Note (opzionale)</h2>
        <textarea
          value={input.notes}
          onChange={(e) => handleChange('notes', e.target.value)}
          placeholder="Contesto aggiuntivo sull'organizzazione, priorità KORA, specificità del programma welfare..."
          rows={3}
      ...
      {/* ── SECTION F: Live Validation ── */}
      {liveValidation && (liveValidation.errors.length > 0 || liveValidation.warnings.length > 0) && (
        <section className="space-y-2 border-t border-slate-100 pt-4">
          <p className="text-xs font-semibold text-slate-600 uppercase tracking-wide">Validazione</p>
      ...
      <section className="border-t border-slate-100 pt-6">
        ...
        >
          Genera Bozza Azienda
        </button>
        {liveValidation && !liveValidation.is_valid && (
          <p className="mt-2 text-xs text-slate-400">Correggi gli errori prima di generare la bozza.</p>
        )}
      </section>

      {/* ── SECTION H: Draft Preview ── */}
      {showDraft && draft && (
        <section className="space-y-5 border-t border-slate-100 pt-6">
          <div>
            ...
            <p className="text-[10px] text-slate-400 font-mono mt-0.5">{draft.draft_id}</p>
          </div>
          ...
          <div className="rounded-lg border border-slate-200 bg-white p-4 space-y-3">
            <div className="flex items-start justify-between gap-2">
              ...
              <span className={`rounded border px-2 py-0.5 text-[10px] font-semibold ${
                ...
                {draft.status === 'pipeline_ready' ? 'Pipeline Ready' : 'Bozza'}
              </span>
            </div>

            <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 text-[10px]">
              <div>
                <p className="text-slate-400 font-semibold uppercase tracking-wide">Settore</p>
                <p className="text-slate-700 mt-0.5 capitalize">{draft.input.sector.replace(/_/g, ' ')}</p>
              </div>
              <div>
                <p className="text-slate-400 font-semibold uppercase tracking-wide">Dipendenti</p>
                <p className="text-slate-700 mt-0.5">{draft.input.headcount}</p>
              </div>
              <div>
                <p className="text-slate-400 font-semibold uppercase tracking-wide">Dimensione</p>
                <p className="text-slate-700 mt-0.5">{SIZE_BAND_LABELS[draft.workforce_preview.size_band] ?? draft.workforce_preview.size_band}</p>
              </div>
              <div>
                <p className="text-slate-400 font-semibold uppercase tracking-wide">Anno</p>
                <p className="text-slate-700 mt-0.5">{draft.input.reporting_year}</p>
              </div>
            </div>

            {draft.template && (
              <div className="rounded bg-slate-50 px-3 py-2 space-y-1">
                <p className="text-[9px] font-semibold uppercase tracking-wide text-slate-400">Template selezionato</p>
                <p className="text-xs font-semibold text-slate-700">{draft.template.label}</p>
                <div className="flex flex-wrap gap-1 mt-1">
                  {draft.template.suggested_pillars.map((p) => (
                    <span key={p} className="rounded bg-white border border-slate-200 px-1.5 py-0.5 text-[8px] font-mono text-slate-500">{p}</span>
                  ))}
                </div>
              </div>
            )}
          ...
          <div className="rounded-lg border border-blue-200 bg-blue-50/40 p-4 space-y-2">
            <p className="text-[10px] font-semibold uppercase tracking-wide text-blue-500">
              Workforce Baseline Preview
            </p>
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 text-[10px]">
              <div>
                <p className="text-slate-400">Headcount</p>
                <p className="text-slate-700 font-semibold">{draft.workforce_preview.headcount}</p>
              </div>
              <div>
                <p className="text-slate-400">Dimensione</p>
                <p className="text-slate-700 font-semibold">{SIZE_BAND_LABELS[draft.workforce_preview.size_band] ?? draft.workforce_preview.size_band}</p>
              </div>
              <div>
                <p className="text-slate-400">Pipeline eligibile</p>
                <p className={`font-semibold ${draft.workforce_preview.eligible_for_pipeline ? 'text-emerald-600' : 'text-rose-600'}`}>
                  {draft.workforce_preview.eligible_for_pipeline ? 'Sì' : 'No'}
                </p>
              </div>
            </div>
            <p className="text-[10px] text-blue-700 leading-relaxed">{draft.workforce_preview.cluster_note}</p>
            <p className="text-[9px] text-blue-600 italic">{draft.workforce_preview.privacy_threshold_note}</p>
          </div>

          {/* Pipeline handoff */}
          <div className="rounded-lg border border-slate-200 bg-white p-4 space-y-3">
            <p className="text-[10px] font-semibold uppercase tracking-wide text-slate-400">
              Pipeline KORA — Prossimi Step
            </p>
              ...
              {draft.pipeline_handoff.map((link, i) => (
                <div
                  ...
                  className={`flex items-center gap-3 rounded p-2 ${
                    link.available ? 'bg-slate-50' : 'bg-slate-50/50 opacity-60'
                  }`}
                >
          ...
          {/* Methodology note */}
          <div className="rounded border border-slate-100 bg-slate-50 px-3 py-2 text-[10px] text-slate-400 leading-relaxed space-y-1">
            <p>
            ...
            <p>
              <span className="font-mono">demo_session_only: true · production_ready: false · synthetic_demo_data: true</span>
            </p>
          </div>
        </section>
      )}
