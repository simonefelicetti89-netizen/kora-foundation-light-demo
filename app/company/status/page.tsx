...
// C-04: Company Status Center — live-only: richiede sessione company autenticata.
// Risponde a: Dove sono? / Cosa devo fare? / I miei dati sono stati ricevuti?
// Dati dalla sessione Supabase. Nessun dato sintetico. Nessun branch demo.
...
export default function CompanyStatusCenterPage() {
  ...
  // ── Loading guard — MUST precede any data rendering ──────────────────────
  ...
  // ── Derived state ─────────────────────────────────────────────────────────
  const readiness  = liveReadiness ?? DEFAULT_READINESS;
  const submissions = liveSubmissions;
  const companyName = liveCompanyName ?? 'La tua organizzazione';

  const pipeline   = derivePipelineStatus(readiness, submissions);
  const checklist  = deriveChecklist(readiness, submissions, 0);
  ...
  return (
    <div style={{ maxWidth: 880, padding: '24px 24px 48px' }}>
      ...
      {/* ── SECTION A — Clarification alert ────────────────────────────────── */}
      {hasClarification && (
        <div style={{
          marginBottom: 20, padding: '16px 20px', borderRadius: 10,
          background: 'rgba(245,158,11,0.10)', border: '2px solid rgba(245,158,11,0.50)',
        }}>
          <div className="flex items-start gap-3">
            <div style={{
            ...
            <div className="flex-1">
              ...
              <Link
                ...
                style={{
                  ...
                  border: '1px solid rgba(245,158,11,0.50)', borderRadius: 6,
      ...
      <div style={{ marginTop: 16 }}>
        ...
        >
          ...
          <div style={{ marginTop: 12 }}>
            <Link
              href="/company/workspace#data-submission"
              style={{ fontSize: 12, fontWeight: 600, color: 'rgba(74,127,224,0.80)', textDecoration: 'none' }}
            >
      ...
      {/* ── SECTION F — Worker Space (live: dati non ancora disponibili) ──── */}
      <div style={{ marginTop: 16 }}>
        <SectionCard title="Worker Space — stato aggregato">
          <div style={{ padding: '12px 0', fontSize: 13, color: 'rgba(6,3,43,0.45)' }}>
            I dati aggregati del Worker Space saranno disponibili dopo la configurazione del roster.
            Contatta KORA Admin per abilitare My KORA per i tuoi lavoratori.
          </div>
          <div style={{
            ...
            Soglia privacy N≥10 applicata su tutti i segmenti. Nessun dato individuale visibile.
          </div>
        </SectionCard>
      </div>
