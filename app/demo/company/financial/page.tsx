'use client';
// B130: Demo-only Financial Governance — dati sintetici Meridiana / Ferretti.
// Mostra struttura completa BTI™ Engine su dati demo sintetici.
// Non richiede sessione company autenticata.
...
export default function FinancialGovernanceDemo() {
  ...
  const { data: liveScoring } = useScoringResult({
    tenantId:   COMPANY_ID,
  ...
  if (!result.allowed) {
    return (
      <div className="space-y-5">
        <PageMasthead eyebrow="Governance finanziaria" title="Governance finanziaria" subline="Lettura informativa del rapporto tra budget people, attivazione profonda e opportunità di riallocazione." />
  ...
  if (!rec) {
    return (
      <div className="space-y-5">
        <PageMasthead eyebrow="Governance finanziaria" title="Governance finanziaria" subline="Lettura informativa del rapporto tra budget people, attivazione profonda e opportunità di riallocazione." />
        <p style={{ fontSize: '13px', color: TOKENS.inkHint }}>Nessun dato di governance finanziaria disponibile per questo scenario.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ── 1. PageMasthead ────────────────────────────────────────────────── */}
      <BoundaryBadge mode="DEMO" variant="light" suffix="· Meridiana" style={{ marginBottom: 6 }} />
      <PageMasthead
        eyebrow={`Governance Finanziaria · DEMO · ${activeScenario} · ${rec.reporting_period}`}
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
        <FinCard label="Budget allocato"   value={eur(rec.budget_allocated_total)} sub={rec.currency} note="Budget allocato ≠ Budget attivato" />
        <FinCard label="Budget utilizzato" value={eur(rec.budget_used_total)} sub={`${(rec.budget_utilization_rate * 100).toFixed(0)}% utilizzo`} accent={utilizationAbove70} />
        <FinCard label="Impegnato"         value={eur(rec.budget_committed_total)} sub="in attesa di conferma" />
        <FinCard label="Residuo"           value={eur(rec.budget_residual)} sub="non convertito in attivazione" note="Componente dell'Activation Debt" />
        <FinCard label="Costo per IU"      value={`€${rec.cost_per_iu_indicator}`} sub="per IU verificata" note="Solo attivazioni budget-mediated" />
        <FinCard label="Quota benefit monetari" value={btiRecord ? pct(btiRecord.economic_relief_share) : '—'} sub="spesa che non genera IU" note="Non è spesa sbagliata — può diventare più intelligente" />
      </div>

      {/* ── 4. Utilizzo + narrative ────────────────────────────────────────── */}
      ...
      <div style={{ background: TOKENS.surface, border: TOKENS.cardBorder, borderRadius: TOKENS.cardRadius, overflow: 'hidden' }}>
        <table className="w-full">
          <TableHead cols={[{ label: 'Categoria' }, { label: 'Cosa include' }, { label: 'Natura' }, { label: 'Perimetro' }]} />
          <tbody>
            {[
      ...
      {/* ── 6. Budget-to-Human-Impact doctrine ────────────────────────────── */}
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        <FinCard label="Budget people / welfare" value={btiRecord ? eur(btiRecord.total_people_welfare_budget) : '—'} sub="welfare, formazione, iniziative people" note="Budget allocated ≠ Budget activated" />
        <FinCard label="Quota benefit monetari" value={btiRecord ? pct(btiRecord.economic_relief_share) : '—'} sub="spesa che non genera Impact Units" note="Non è spesa sbagliata — può diventare più intelligente" />
        <FinCard
          label="Costo per Impact Unit™"
        ...
        />
        <FinCard label="Activation Debt™" value={btiRecord ? eur(btiRecord.activation_debt_eur) : '—'} sub="budget non convertito in attivazione" note="Stima direzionale — non garantito" />
      </div>

      {/* Executive Narrative */}
      {btiIntelligence && (
        <div style={{ background: TOKENS.surface, border: `1px solid ${TOKENS.accent}22`, borderRadius: TOKENS.cardRadius, padding: '1.25rem 1.5rem' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 12, paddingTop: 10, borderTop: TOKENS.cardBorder }}>
            <span style={{ fontSize: '11px', color: TOKENS.inkHint }}>Pillar Investment Balance:</span>
            <span style={{
              ...
              background: btiIntelligence.pillarInvestmentBalanceSignal === 'bilanciato'
                ? TOKENS.safeguard.pass.bg : btiIntelligence.pillarInvestmentBalanceSignal === 'moderato'
                  ? TOKENS.safeguard.watch.bg : TOKENS.safeguard.cap.bg,
              color: btiIntelligence.pillarInvestmentBalanceSignal === 'bilanciato'
                ? TOKENS.safeguard.pass.text : btiIntelligence.pillarInvestmentBalanceSignal === 'moderato'
                  ? TOKENS.safeguard.watch.text : TOKENS.safeguard.cap.text,
              borderRadius: 4, padding: '2px 8px',
            }}>
              {btiIntelligence.pillarInvestmentBalanceSignal}
            </span>
            <span style={{ fontSize: '11px', color: TOKENS.inkHint }}>
              ({btiRecord ? Math.round(btiRecord.pillar_investment_balance * 100) : '—'}/100) · segnale direzionale
            </span>
          </div>
        </div>
      )}
      ...
      {btiRecord?.activation_debt_description_it && (
        <div style={{ background: TOKENS.surface, border: TOKENS.cardBorder, borderRadius: TOKENS.cardRadius, padding: '1rem 1.25rem', fontSize: '12px', color: TOKENS.inkSecondary, lineHeight: 1.7 }}>
          <span style={{ fontWeight: 600, color: TOKENS.ink }}>Activation Debt — dettaglio: </span>
          {btiRecord.activation_debt_description_it}
        </div>
      )}

      {/* Reallocation WHY Trace */}
      {btiIntelligence && btiIntelligence.reallocationAnalysis.reasons.length > 0 && (
        <div style={{ background: TOKENS.surface, border: TOKENS.cardBorder, borderRadius: TOKENS.cardRadius, padding: '1.125rem 1.25rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {btiIntelligence.reallocationAnalysis.reasons.map((r, i) => (
              <div key={i} style={{ display: 'flex', gap: 12, alignItems: 'flex-start', background: r.contribution === 'primary' ? 'rgba(199,111,61,0.04)' : 'rgba(6,3,43,0.02)', border: r.contribution === 'primary' ? '1px solid rgba(199,111,61,0.14)' : TOKENS.cardBorder, borderRadius: 8, padding: '10px 12px' }}>
                <span style={{ flexShrink: 0, marginTop: 1, fontSize: '10px', fontWeight: 600, fontFamily: 'var(--font-jakarta)', background: r.contribution === 'primary' ? `${TOKENS.accent}18` : TOKENS.inkBorder, color: r.contribution === 'primary' ? TOKENS.accent : TOKENS.inkSecondary, borderRadius: 4, padding: '2px 7px' }}>
      ...
      {btiRecord?.non_budget_mediated_activation_note && (
        <div style={{ background: 'rgba(199,111,61,0.05)', border: '1px solid rgba(199,111,61,0.14)', borderRadius: TOKENS.cardRadius, padding: '1rem 1.25rem', fontSize: '12px', color: TOKENS.ink, lineHeight: 1.7 }}>
              ...
              <div key={pillar}>
                <div className="flex justify-between items-baseline mb-1.5">
                  ...
                  <span style={{ fontFamily: 'var(--font-jakarta)', fontWeight: 700, fontSize: '13px', color: rank === 0 ? TOKENS.accent : TOKENS.ink, fontVariantNumeric: 'tabular-nums' }}>
                    {budget > 0 ? eur(budget) : '—'}
                    <span style={{ fontWeight: 400, color: TOKENS.inkHint, marginLeft: 6 }}>{budget > 0 ? `(${(share * 100).toFixed(0)}%)` : ''}</span>
                  </span>
                </div>
      ...
      <div style={{ background: TOKENS.surface, border: TOKENS.cardBorder, borderRadius: TOKENS.cardRadius, overflow: 'hidden' }}>
        <table className="w-full">
          <TableHead cols={[{ label: 'Pillar' }, { label: 'Budget spend', align: 'right' }, { label: '%', align: 'right' }, { label: 'Attivazione profonda', align: 'right' }, { label: 'Debt', align: 'center' }, { label: 'Raccomandazione direzionale' }]} />
          <tbody>
            {pillarOrder.map((pillar, i) => {
              ...
              return (
                <tr key={pillar} style={{ background: i % 2 === 0 ? TOKENS.surface : 'rgba(6,3,43,0.02)', borderBottom: TOKENS.cardBorder }}>
                  ...
                  <td style={{ padding: '12px 14px', textAlign: 'right', verticalAlign: 'top', fontFamily: 'var(--font-jakarta)', fontWeight: 600, fontSize: '12px', color: TOKENS.ink, fontVariantNumeric: 'tabular-nums', whiteSpace: 'nowrap' }}>{budget > 0 ? eur(budget) : '—'}</td>
                  <td style={{ padding: '12px 14px', textAlign: 'right', verticalAlign: 'top', fontFamily: 'var(--font-jakarta)', fontSize: '12px', color: TOKENS.inkSecondary, fontVariantNumeric: 'tabular-nums' }}>{budget > 0 ? `${(share * 100).toFixed(0)}%` : '—'}</td>
                  <td style={{ padding: '12px 14px', textAlign: 'right', verticalAlign: 'top', fontFamily: 'var(--font-jakarta)', fontWeight: 600, fontSize: '12px', color: TOKENS.ink, fontVariantNumeric: 'tabular-nums', whiteSpace: 'nowrap' }}>{deep > 0 ? eur(deep) : '—'}</td>
                  <td style={{ padding: '12px 14px', textAlign: 'center', verticalAlign: 'top' }}><Pill label={dt.label} bg={dt.bg} text={dt.text} /></td>
                  <td style={{ padding: '12px 14px', verticalAlign: 'top', fontSize: '11px', color: TOKENS.inkSecondary, lineHeight: 1.55 }}>{staticData?.recommendation ?? '—'}</td>
                </tr>
              );
            })}
          </tbody>
        ...
        <div style={{ background: TOKENS.surface, border: TOKENS.cardBorder, borderRadius: TOKENS.cardRadius, overflow: 'hidden' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 0 }}>
            {btiIntelligence.pillarClassifications.map((pc, i) => {
              ...
              return (
                <div key={pc.pillar} style={{ padding: '12px 14px', borderRight: i < 4 ? TOKENS.cardBorder : 'none', background: i % 2 === 0 ? TOKENS.surface : 'rgba(6,3,43,0.015)' }}>
                  ...
                  <span style={{ display: 'inline-block', marginBottom: 6, fontSize: '10px', fontWeight: 600, fontFamily: 'var(--font-jakarta)', background: sc.bg, color: sc.text, borderRadius: 4, padding: '2px 7px' }}>
                    {sc.label}
                  </span>
        ...
        return (
          <div style={{ background: TOKENS.surface, border: TOKENS.cardBorder, borderRadius: TOKENS.cardRadius, overflow: 'hidden' }}>
            <div style={{ padding: '0.875rem 1.25rem', borderBottom: TOKENS.cardBorder, display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
              <p style={{ fontFamily: 'var(--font-jakarta)', fontWeight: 700, fontSize: '13px', color: TOKENS.ink }}>LIFE Diversity Intelligence™</p>
              <span style={{ fontSize: '10px', fontWeight: 500, fontFamily: 'var(--font-jakarta)', background: TOKENS.inkBorder, color: TOKENS.inkSecondary, borderRadius: 4, padding: '2px 7px' }}>regola · non AI</span>
              <span style={{ fontSize: '10px', fontWeight: 600, fontFamily: 'var(--font-jakarta)', background: ct.bg, color: ct.text, borderRadius: 4, padding: '2px 8px' }}>{ct.label}</span>
                ...
                {ld.activeSubcategories.length === 0 ? (
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                    {ld.activeSubcategories.map((code) => {
                      ...
                      return (
                        <span key={code} style={{ fontSize: '10px', fontWeight: isDominant ? 700 : 500, background: isDominant ? `${TOKENS.accent}14` : TOKENS.inkBorder, color: isDominant ? TOKENS.accent : TOKENS.inkSecondary, border: isDominant ? `1px solid ${TOKENS.accent}30` : '1px solid transparent', borderRadius: 4, padding: '2px 8px' }} title={meta.description}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {ld.recommendations.slice(0, 2).map((rec) => (
                      <div key={rec.id} style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
                        <span style={{ flexShrink: 0, marginTop: 1, fontSize: '9px', fontWeight: 700, fontFamily: 'var(--font-jakarta)', background: rec.priority === 'alta' ? `${TOKENS.accent}18` : TOKENS.inkBorder, color: rec.priority === 'alta' ? TOKENS.accent : TOKENS.inkSecondary, borderRadius: 3, padding: '1px 5px', textTransform: 'uppercase' as const, letterSpacing: '0.04em' }}>
                          {rec.priority}
                        </span>
            ...
            {ld.privacyWarningLevel !== 'none' && ld.privacyWarningMessage && (
              <div style={{ margin: '0 1.25rem 1rem', padding: '8px 12px', borderRadius: 6, background: wt.bg, border: `1px solid ${wt.border}` }}>
                <p style={{ fontSize: '11px', fontWeight: 600, color: wt.text, lineHeight: 1.55 }}>⚠ Privacy — {ld.privacyWarningMessage}</p>
              </div>
            )}
      ...
      {careEconomy && (() => {
        ...
        return (
          <div style={{ background: TOKENS.surface, border: TOKENS.cardBorder, borderRadius: TOKENS.cardRadius, overflow: 'hidden' }}>
            <div style={{ padding: '0.875rem 1.25rem', borderBottom: TOKENS.cardBorder, display: 'flex', alignItems: 'center', gap: 10 }}>
              <p style={{ fontFamily: 'var(--font-jakarta)', fontWeight: 700, fontSize: '13px', color: TOKENS.ink, flex: 1 }}>Care Economy Intelligence™</p>
              <span style={{ fontSize: '10px', fontWeight: 600, background: tok.bg, color: tok.text, borderRadius: 4, padding: '2px 8px' }}>{tok.label}</span>
            ...
            <div style={{ padding: '1rem 1.25rem' }}>
              <div style={{ marginBottom: '1rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                  ...
                  <p style={{ fontSize: '11px', color: TOKENS.inkSecondary }}>{careEconomy.activeCareSubcategories.length} / 3 aree attive</p>
                </div>
                <div style={{ height: 6, background: 'rgba(6,3,43,0.08)', borderRadius: 3, overflow: 'hidden' }}>
                  <div style={{ height: '100%', width: `${careEconomy.careCoverageScore * 100}%`, background: TOKENS.accent, borderRadius: 3 }} />
                </div>
              </div>
              {(['childcare', 'eldercare_caregiving', 'family_parental_support'] as const).map((sub) => {
                ...
                return (
                  <div key={sub} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 10px', background: isActive ? 'rgba(6,3,43,0.03)' : 'transparent', borderRadius: 6, marginBottom: 4, border: isActive ? '1px solid rgba(6,3,43,0.08)' : '1px solid transparent' }}>
                    <span style={{ fontSize: '13px' }}>{isActive ? '✓' : '○'}</span>
                    <p style={{ fontSize: '12px', color: isActive ? TOKENS.ink : TOKENS.inkHint, fontWeight: isActive ? 500 : 400, flex: 1 }}>{labels[sub]}</p>
                    <span style={{ fontSize: '10px', color: isActive ? TOKENS.safeguard.pass.text : TOKENS.inkHint }}>{isActive ? 'presente' : 'non presente'}</span>
                  </div>
                );
              })}
              <p style={{ marginTop: '1rem', fontSize: '12px', color: TOKENS.inkSecondary, lineHeight: 1.65 }}>{careEconomy.narrative}</p>
              {careEconomy.recommendations.length > 0 && (
                <div style={{ marginTop: '0.75rem', display: 'flex', flexDirection: 'column' as const, gap: 6 }}>
                  {careEconomy.recommendations.map((rec) => (
                    <div key={rec.id} style={{ display: 'flex', gap: 8, padding: '8px 10px', background: 'rgba(6,3,43,0.03)', borderRadius: 6, border: '1px solid rgba(6,3,43,0.07)' }}>
      ...
      <div style={{ background: TOKENS.surface, border: TOKENS.cardBorder, borderRadius: TOKENS.cardRadius, overflow: 'hidden' }}>
        ...
        <table className="w-full">
          <TableHead cols={[{ label: 'KPI People' }, { label: 'Variazione osservata', align: 'right' }, { label: 'Confidenza', align: 'center' }, { label: 'Interpretazione direzionale' }]} />
          <tbody>
              ...
              return (
                <tr key={row.kpi} style={{ background: i % 2 === 0 ? TOKENS.surface : 'rgba(6,3,43,0.02)', borderBottom: TOKENS.cardBorder }}>
                  ...
                  <td style={{ padding: '11px 14px', textAlign: 'right', verticalAlign: 'top', whiteSpace: 'nowrap' }}>
                    <span style={{ fontFamily: 'var(--font-jakarta)', fontWeight: 700, fontSize: '13px', color: dirColor, fontVariantNumeric: 'tabular-nums' }}>{dirArrow} {row.change}</span>
                    <p style={{ fontSize: '10px', color: TOKENS.inkHint, marginTop: 2 }}>{row.period}</p>
                  </td>
                  <td style={{ padding: '11px 14px', textAlign: 'center', verticalAlign: 'top' }}><Pill label={row.confidence} bg={ct.bg} text={ct.text} /></td>
      ...
      <div style={{ background: TOKENS.surface, border: TOKENS.cardBorder, borderRadius: TOKENS.cardRadius, overflow: 'hidden' }}>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table className="min-w-full">
            <thead>
              <tr style={{ borderBottom: `2px solid ${TOKENS.ink}` }}>
                <th style={{ padding: '10px 14px', fontSize: '10px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', color: TOKENS.inkHint, textAlign: 'left', whiteSpace: 'nowrap' }}>Segnale KORA</th>
                {CORR_COLS.map((col) => (
                  <th key={col} style={{ padding: '10px 14px', fontSize: '10px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', color: TOKENS.inkHint, textAlign: 'center', whiteSpace: 'nowrap' }}>{col}</th>
                ))}
              </tr>
            </thead>
            <tbody>
                  ...
                  {CORR_MATRIX[ri].map((strength, ci) => {
                    ...
                    return (
                      <td key={ci} title={cell.tip} style={{ padding: '10px 14px', textAlign: 'center' }}>
                        <span style={{ fontWeight: strength === 'forte' ? 700 : 500, color: cell.color, fontSize: '14px' }}>{cell.symbol}</span>
                      </td>
                    );
                  })}
                </tr>
      ...
      {/* ── 10. Scenari direzionali ────────────────────────────────────────── */}
      <SectionLabel>Scenari direzionali</SectionLabel>
      <div className="grid gap-4 sm:grid-cols-3">
        {SCENARIOS_BTI.map((sc) => (
          <div key={sc.id} style={{ background: TOKENS.surface, border: sc.isActive ? `1px solid ${TOKENS.accent}44` : TOKENS.cardBorder, borderRadius: TOKENS.cardRadius, padding: '1.125rem' }}>
            <div className="flex items-center justify-between gap-2 mb-2">
              <p style={{ fontSize: '12px', fontWeight: 600, color: TOKENS.ink }}>{sc.label}</p>
              <Pill label={sc.isActive ? 'Attuale' : 'Direzionale'} bg={sc.isActive ? `${TOKENS.accent}18` : TOKENS.inkBorder} text={sc.isActive ? TOKENS.accent : TOKENS.inkSecondary} />
            </div>
      ...
      <div style={{ background: TOKENS.surface, border: TOKENS.cardBorder, borderRadius: TOKENS.cardRadius, overflow: 'hidden' }}>
        ...
        <table className="w-full">
          <TableHead cols={[{ label: 'Priorità', align: 'center' }, { label: 'Azione raccomandata' }, { label: 'Budget indicativo', align: 'right' }, { label: 'Segnale atteso' }, { label: 'Rischio', align: 'center' }]} />
          <tbody>
            {INVESTMENT_RECS.map((r, i) => {
              ...
              return (
                <tr key={i} style={{ background: i % 2 === 0 ? TOKENS.surface : 'rgba(6,3,43,0.02)', borderBottom: TOKENS.cardBorder }}>
                  <td style={{ padding: '12px 14px', textAlign: 'center', verticalAlign: 'top' }}><Pill label={r.priority} bg={pt.bg} text={pt.text} /></td>
                  <td style={{ padding: '12px 14px', fontSize: '12px', fontWeight: 500, color: TOKENS.ink, verticalAlign: 'top' }}>{r.action}</td>
                  <td style={{ padding: '12px 14px', textAlign: 'right', fontSize: '12px', color: TOKENS.inkSecondary, fontVariantNumeric: 'tabular-nums', verticalAlign: 'top', whiteSpace: 'nowrap' }}>{r.budget_note}</td>
                  ...
                  <td style={{ padding: '12px 14px', textAlign: 'center', verticalAlign: 'top' }}><Pill label={r.risk} bg={rt.bg} text={rt.text} /></td>
                </tr>
              );
            })}
          </tbody>
      ...
      {/* ── 12. Board Pack CTA ─────────────────────────────────────────────── */}
      <div style={{ background: TOKENS.surface, border: `1px solid ${TOKENS.accent}33`, borderRadius: TOKENS.cardRadius, padding: '1.125rem 1.25rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', flexWrap: 'wrap' }}>
        <div>
      ...
      <div style={{ background: TOKENS.surface, border: TOKENS.cardBorder, borderRadius: TOKENS.cardRadius, padding: '1.25rem' }}>
        <p style={{ fontSize: '12px', fontWeight: 600, color: TOKENS.ink, marginBottom: 10 }}>Dati sintetici — limitazioni obbligatorie</p>
