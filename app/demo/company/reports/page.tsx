'use client';
// B130: Demo-only Decision Pack — dati sintetici Meridiana / Ferretti.
...
function MetricGrid({ metrics }: { metrics: DecisionPackMetric[] }) {
  ...
  return (
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
      {metrics.map((m) => (
        <div key={m.code} style={{ background: TOKENS.surface, border: TOKENS.cardBorder, borderRadius: TOKENS.cardRadius, padding: '0.875rem' }}>
          <p style={{ fontFamily: 'var(--font-jakarta)', fontWeight: 500, fontSize: '10px', letterSpacing: '0.06em', textTransform: 'uppercase', color: TOKENS.inkHint, marginBottom: 6 }}>{m.label}</p>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      {insights.map((ins) => {
        ...
        return (
          <div key={ins.id} style={{ borderLeft: `4px solid ${sv.leftColor}`, borderRadius: '0 6px 6px 0', padding: '10px 14px', background: sv.bg, fontSize: '12px', lineHeight: 1.6 }}>
          ...
          <div key={rec.id} style={{ borderLeft: `4px solid ${pt.leftColor}`, borderRadius: '0 6px 6px 0', padding: '10px 14px', background: pt.bg }}>
            <div className="flex items-start justify-between gap-2">
              ...
              <div className="flex shrink-0 gap-1.5 items-center">
                <span style={{ fontSize: '10px', fontWeight: 700, background: pt.tagBg, color: pt.tagText, borderRadius: 4, padding: '2px 6px' }}>{rec.priority}</span>
                <span style={{ fontSize: '10px', background: TOKENS.inkBorder, color: TOKENS.inkSecondary, borderRadius: 4, padding: '2px 6px' }}>{rec.horizon}</span>
              </div>
            </div>
    ...
    <div id={section.code} className="scroll-mt-24 space-y-4">
      <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
        <div className="flex flex-wrap items-center gap-2">
          <span style={{ fontFamily: 'monospace', fontSize: '10px', background: TOKENS.inkBorder, color: TOKENS.inkSecondary, borderRadius: 4, padding: '2px 6px' }}>{section.code}</span>
          {section.audience.map((a) => (
            <span key={a} style={{ fontSize: '10px', background: `${TOKENS.accent}12`, color: TOKENS.accent, borderRadius: 4, padding: '2px 6px', border: `1px solid ${TOKENS.accent}22` }}>{a}</span>
          ))}
        </div>
...
function DeltaCard({ delta }: { delta: DecisionPackMetricDelta }) {
  ...
  return (
    <div style={{ background: tt.bg, border: `1px solid ${tt.borderColor}`, borderRadius: TOKENS.cardRadius, padding: '10px 12px' }}>
      <p style={{ fontFamily: 'var(--font-jakarta)', fontSize: '9px', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: TOKENS.inkHint, marginBottom: 6 }}>{delta.label}</p>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div style={{
            ...
            <p style={{ fontWeight: 600 }}>
              {comparison.methodology_comparable ? 'Confronto diretto valido — stessa metodologia' : 'Confronto indicativo — metodologia cambiata tra i periodi'}
            </p>
...
function VersionCard({ version, isLatest }: { version: DecisionPackVersion; isLatest: boolean }) {
  ...
  return (
    <div style={{
      ...
      borderRadius: TOKENS.cardRadius, padding: '1rem', display: 'flex', flexDirection: 'column', gap: 12,
    }}>
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {isLatest && (
              <span style={{ fontSize: '9px', fontWeight: 700, background: TOKENS.accent, color: '#FFFFFF', borderRadius: 9999, padding: '2px 7px', letterSpacing: '0.06em', textTransform: 'uppercase' }}>Corrente</span>
            )}
          </div>
          <p style={{ fontSize: '13px', fontWeight: 600, color: TOKENS.ink, marginTop: 4 }}>{version.title ?? `Decision Pack · ${version.period}`}</p>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        {version.kora_index_value !== null && version.kora_index_value !== undefined ? (
          <div>
            ...
            <p style={{ fontFamily: 'var(--font-jakarta)', fontWeight: 700, fontSize: '1.5rem', color: TOKENS.accent, lineHeight: 1, marginTop: 3, fontVariantNumeric: 'tabular-nums' }}>{version.kora_index_value}</p>
          </div>
        ) : (
          <div>
            <p style={{ fontSize: '10px', color: TOKENS.inkHint }}>KORA Index</p>
      ...
      {version.change_summary && (
        <p style={{ fontSize: '11px', color: TOKENS.inkSecondary, lineHeight: 1.6, borderTop: TOKENS.cardBorder, paddingTop: 10 }}>{version.change_summary}</p>
      )}
...
const PILLAR_RAMP = [TOKENS.accent, 'rgba(6,3,43,0.65)', 'rgba(6,3,43,0.50)', 'rgba(6,3,43,0.35)', 'rgba(6,3,43,0.22)'];
...
// B130 Demo-only: Decision Pack sintetico Meridiana / Ferretti
  ...
  return (
    <div className="space-y-6">
      ...
      <div style={{ background: TOKENS.surface, border: TOKENS.cardBorder, borderRadius: TOKENS.cardRadius, padding: '1.25rem 1.5rem' }}>
        <p style={{ fontSize: '13px', fontWeight: 600, color: TOKENS.ink, marginBottom: 8 }}>Lettura direzionale — non certificativa</p>
        <p style={{ fontSize: '13px', color: TOKENS.inkSecondary, lineHeight: 1.7 }}>
          ...
          Il Decision Pack è un supporto informativo per il confronto interno — non una certificazione ESG, non un report regolatorio automatico, non un&apos;attestazione pubblica.
          Serve a HR, finance, ESG e board per leggere e discutere l&apos;attivazione umana dell&apos;organizzazione.
        </p>
      </div>

      {/* ── Primary Decision Pack Card ─────────────────────────────────────── */}
      <div style={{ background: TOKENS.surface, border: `1px solid ${TOKENS.accent}44`, borderLeft: `4px solid ${TOKENS.accent}`, borderRadius: TOKENS.cardRadius, padding: '1.5rem' }}>
        <div className="flex items-start justify-between gap-4 flex-wrap mb-5">
          <div>
            ...
            <p style={{ fontSize: '12px', color: TOKENS.inkSecondary, marginTop: 4 }}>Output direzionale company-scoped · KORA Foundation Light · pre-empirical calibration</p>
          </div>
        ...
        {hasFullReport && koraIndex ? (
          <div className="grid grid-cols-1 gap-4 mb-5 sm:grid-cols-3">
            <div style={{ background: TOKENS.inkBorder, borderRadius: 10, padding: '1rem', textAlign: 'center' }}>
              ...
              <p style={{ fontFamily: 'var(--font-jakarta)', fontWeight: 700, fontSize: '2.5rem', color: TOKENS.ink, lineHeight: 1, fontVariantNumeric: 'tabular-nums' }}>{koraIndex.kora_index_value}</p>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <a href="/api/company/decision-pack" target="_blank" rel="noopener noreferrer"
          ...
          <span style={{ fontSize: '11px', color: TOKENS.inkHint }}>Usa Stampa / Salva come PDF dal browser (Cmd+P)</span>
        </div>
      </div>

      {/* ── Stato Decision Pack ─────────────────────────────────────────────── */}
      <SectionLabel>Stato Decision Pack</SectionLabel>
      <div style={{ background: TOKENS.safeguard.watch.bg, border: `1px solid ${TOKENS.safeguard.watch.dot}44`, borderRadius: TOKENS.cardRadius, padding: '1rem 1.25rem', display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}>
        <div>
          ...
          <p style={{ fontSize: '13px', fontWeight: 600, color: TOKENS.safeguard.watch.text }}>Bozza disponibile — revisione advisor richiesta</p>
      ...
      {versionHistory.length > 0 && (
        <>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {versionHistory.map((version, idx) => <VersionCard key={version.version_id} version={version} isLatest={idx === 0} />)}
          </div>
        ...
        <>
          ...
          <div className="overflow-x-auto print:hidden">
            <div className="flex gap-2 min-w-max pb-1">
              {SECTION_NAV.map((nav) => (
                <a key={nav.id} href={`#${nav.id}`} style={{ borderRadius: 6, border: TOKENS.cardBorder, background: TOKENS.surface, padding: '6px 14px', fontSize: '11.5px', fontWeight: 500, color: TOKENS.inkSecondary, textDecoration: 'none', whiteSpace: 'nowrap' }}>
                  {nav.label}
                </a>
              ))}
            </div>
      ...
      <div style={{ background: TOKENS.surface, border: TOKENS.cardBorder, borderRadius: TOKENS.cardRadius, padding: '1.25rem' }}>
        ...
        <a href="/api/company/decision-pack" target="_blank" rel="noopener noreferrer"
          ...
          Apri Board Pack →
          <span style={{ fontWeight: 400, fontSize: '10px', color: 'rgba(244,241,233,0.60)' }}>Stampa / Salva PDF dal browser (Cmd+P)</span>
        </a>
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
          {exportActions.map((action) => (
            <div key={action.action_id ?? action.label} style={{ borderRadius: TOKENS.cardRadius, border: action.enabled ? `1px solid ${TOKENS.accent}33` : TOKENS.cardBorder, background: action.enabled ? `${TOKENS.accent}08` : TOKENS.inkBorder, padding: '0.875rem', textAlign: 'center', opacity: action.enabled ? 1 : 0.6 }}>
              <p style={{ fontSize: '12px', fontWeight: 500, color: TOKENS.ink }}>{action.label}</p>
              {action.future_capability && (
                <span style={{ display: 'inline-block', marginTop: 6, fontSize: '9px', fontWeight: 700, background: TOKENS.inkBorder, color: TOKENS.inkSecondary, borderRadius: 9999, padding: '2px 7px', letterSpacing: '0.06em', textTransform: 'uppercase' }}>In arrivo</span>
              )}
              {!action.future_capability && !action.enabled && (
                <span style={{ display: 'inline-block', marginTop: 6, fontSize: '9px', fontWeight: 700, background: TOKENS.inkBorder, color: TOKENS.inkHint, borderRadius: 9999, padding: '2px 7px', letterSpacing: '0.06em', textTransform: 'uppercase' }}>Demo</span>
              )}
      ...
      {/* ── Report body — 8 sezioni canoniche ─────────────────────────────────── */}
      {hasFullReport && pack && (
        <div style={{ borderTop: `2px solid ${TOKENS.ink}`, paddingTop: 24, display: 'flex', flexDirection: 'column', gap: 32 }}>
          ...
          {sectionMap.kora_index_v3 && (
            <>
              ...
              {sectionMap.dynamic_scoring_preview && (
                <details style={{ borderRadius: TOKENS.cardRadius, border: TOKENS.cardBorder, background: TOKENS.inkBorder, padding: '0.875rem 1rem', marginTop: 8 }}>
            ...
            <>
              <div id="pillar_analysis" className="scroll-mt-24" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                <p style={{ fontSize: '10px', color: TOKENS.inkHint }}>
                  Aggregato aziendale canonico Q1–Q3 2025 · Dati sintetici demo · synthetic_demo_data: true
                </p>
          ...
          {sectionMap.methodology_boundaries && <SectionBlock section={sectionMap.methodology_boundaries} />}
      ...
      <div style={{ background: TOKENS.surface, border: TOKENS.cardBorder, borderRadius: TOKENS.cardRadius, padding: '1.25rem' }}>
        <p style={{ fontSize: '13px', fontWeight: 600, color: TOKENS.ink, marginBottom: 12 }}>Decision Pack misura l&apos;organizzazione, non gli individui.</p>
        <ul style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {[
            ...
            'Confidence Score: indicatore esterno di affidabilità dati, peso = 0 nel KORA Index v1.0.',
            ...
            <li key={note} style={{ display: 'flex', gap: 8, fontSize: '12px', color: TOKENS.inkSecondary, lineHeight: 1.65 }}>
              <span style={{ flexShrink: 0, color: TOKENS.inkHint, marginTop: 2 }}>·</span>{note}
            </li>
          ))}
        </ul>
      ...
      <div style={{ background: TOKENS.surface, border: `1px solid ${TOKENS.accent}33`, borderRadius: TOKENS.cardRadius, padding: '1.25rem' }}>
        <div className="flex flex-wrap items-center gap-2 mb-4">
          <span style={{ fontSize: '10px', fontWeight: 700, background: `${TOKENS.accent}14`, color: TOKENS.accent, borderRadius: 4, padding: '2px 7px' }}>Indicatore Companion</span>
          <span style={{ fontFamily: 'monospace', fontSize: '10px', color: TOKENS.inkHint }}>not_kora_index_component: true</span>
          <span style={{ fontFamily: 'monospace', fontSize: '10px', color: TOKENS.inkHint }}>provisional_companion_indicator</span>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3 mb-4">
          {(['verified', 'partial', 'self_declared'] as const).map((key) => {
            const label = key === 'verified' ? 'Verificata' : key === 'partial' ? 'Parziale' : 'Autodichiarata';
            const color = key === 'verified' ? TOKENS.safeguard.pass.dot : key === 'partial' ? TOKENS.safeguard.watch.dot : TOKENS.inkHint;
        ...
        <div style={{ borderTop: TOKENS.cardBorder, paddingTop: 12, display: 'flex', flexWrap: 'wrap', gap: 8 }}>
          {(['IMPACT', 'CONNECTION', 'LEGACY'] as const).map((p) => (
            <span key={p} style={{ fontSize: '11px', background: TOKENS.inkBorder, color: TOKENS.inkSecondary, borderRadius: 4, padding: '3px 8px', fontVariantNumeric: 'tabular-nums' }}>
              {p}: {(contributionV2.pillarBreakdown[p] ?? 0).toFixed(2)} IU
            </span>
          ))}
          <span style={{ fontSize: '10px', color: TOKENS.inkHint, fontStyle: 'italic', alignSelf: 'center', marginLeft: 4 }}>distribuzione IU per pillar contribution</span>
        </div>
      </div>
      ...
      <p style={{ fontSize: '10px', color: TOKENS.inkHint, marginTop: 4 }}>
        synthetic_demo_data: true · scenario: {activeScenario} · azienda: {COMPANY_ID}
      </p>
    </div>
  );
}
