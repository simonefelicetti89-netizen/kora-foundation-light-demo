'use client';
// B130: Demo-only Pillar Intelligence — dati sintetici Meridiana / Ferretti.
...
export default function DemoPillarsInitiatives() {
  ...
  const programs    = demoDataService.getPrograms(COMPANY_ID);
  const initiatives = koraContributionService.getCollectiveInitiatives(COMPANY_ID, activeScenario);
    ...
    <div className="space-y-5">
      ...
      <div style={{ background: TOKENS.surface, border: TOKENS.cardBorder, borderRadius: TOKENS.cardRadius, overflow: 'hidden' }}>
          ...
          <tbody>
            {programs.map((prog) => {
              ...
              return (
                <tr
                  key={prog.id}
                  style={{ borderBottom: TOKENS.cardBorder, background: isBlocked ? TOKENS.safeguard.cap.bg : undefined }}
                >
        ...
        <p style={{ fontSize: '11px', color: TOKENS.inkHint, padding: '10px 16px', borderTop: TOKENS.cardBorder }}>
          ...
          <span style={{ fontStyle: 'italic', marginLeft: 8 }}>synthetic_demo_data: true</span>
        </p>
      </div>
