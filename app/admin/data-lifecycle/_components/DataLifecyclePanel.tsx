    ...
    <div className="max-w-5xl mx-auto py-6 px-3 space-y-5">
      ...
      <div className="grid gap-5 lg:grid-cols-[320px_1fr]">
        ...
        <div className="space-y-2">
          <p className="text-xs font-bold text-slate-500 uppercase tracking-wide">Recent Batches</p>
          {batchesLoading && <p className="text-xs text-slate-400">Loading…</p>}
        ...
        <div className="space-y-4">
          ...
          {inspecting && <p className="text-xs text-slate-400">Loading inspection…</p>}
          ...
          {inspect?.ok && inspect.counts && (
            <div className="rounded-lg border border-slate-200 bg-white px-5 py-4 space-y-4">
              ...
              <div className="flex items-start justify-between flex-wrap gap-2">
                <div>
                  <p className="text-sm font-bold text-slate-800">{inspect.tenant?.companyName}</p>
                  <p className="text-xs font-mono text-slate-500">{inspect.tenant?.tenantCode} · {inspect.batch?.reportingPeriod}</p>
                </div>
              ...
              <div className="grid grid-cols-2 gap-x-8 gap-y-1.5 text-xs">
                <span className="text-slate-400">Uploaded Records <strong className="text-slate-800 tabular-nums">{inspect.counts.uploadedRecords}</strong></span>
                <span className="text-slate-400">UEF Records <strong className="text-slate-800 tabular-nums">{inspect.counts.uefRecords}</strong></span>
                <span className="text-slate-400">Approved UEF <strong className="text-slate-800 tabular-nums">{inspect.counts.uefApproved}</strong></span>
                <span className="text-slate-400">KORA Index Results <strong className="text-slate-800 tabular-nums">{inspect.counts.koraIndexResults}</strong></span>
                <span className="text-slate-400">Decision Packs <strong className="text-slate-800 tabular-nums">{inspect.counts.decisionPacks}</strong></span>
              </div>

              {/* Decision Packs */}
              {inspect.decisionPacks && inspect.decisionPacks.length > 0 && (
                <div>
                  <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide mb-1">Decision Packs</p>
                  {inspect.decisionPacks.map(dp => (
                    <div key={dp.id} className="flex items-center justify-between py-1 border-t border-slate-100 text-xs">
                      <span className="font-mono text-slate-500 truncate">{dp.versionId.slice(0,30)}…</span>
              ...
              <div className="pt-2 border-t border-slate-100 space-y-3">
                ...
                <div>
                  <button onClick={handleArchive} disabled={archiveStatus==='loading'}
                    className="rounded border border-slate-300 bg-slate-50 px-4 py-1.5 text-xs font-medium text-slate-700 hover:bg-slate-100 disabled:opacity-50 transition-colors">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-[10px] text-slate-400">Permanently deletes uploaded_record + uef_record. Type DELETE_BATCH to confirm.</p>
                  <div className="flex gap-2 flex-wrap">
                  ...
                  {deleteStatus==='error' && deleteResult && (
                    <div className="text-[10px] text-red-600">
                      ...
                      {deleteResult.hint && <p className="text-slate-500 mt-0.5">{deleteResult.hint}</p>}
                    </div>
                  )}
                </div>
