...
const SUBMISSION_STATUS_CLS: Record<string, string> = {
  submission_accepted:            'border-[rgba(47,125,85,0.22)] bg-[rgba(47,125,85,0.08)] text-kora-success',
  submission_needs_clarification: 'border-[rgba(217,154,43,0.25)] bg-[rgba(217,154,43,0.08)] text-kora-warning-text',
...
const SAFEGUARD_CLS: Record<string, string> = {
  CLEAR:   'border-[rgba(47,125,85,0.22)] bg-[rgba(47,125,85,0.08)] text-kora-success',
  WARNING: 'border-[rgba(217,154,43,0.25)] bg-[rgba(217,154,43,0.08)] text-kora-warning-text',
  FLAGGED: 'border-[rgba(158,59,47,0.22)] bg-[rgba(158,59,47,0.06)] text-kora-critical',
};
  ...
  return (
    <div className="max-w-3xl mx-auto py-6 px-3 space-y-5">
      ...
      >
        ...
        <Link href="/admin/demo/acme-001" className="ml-auto text-[10px] underline hover:text-kora-warning-text shrink-0">
          ← Demo Hub
        </Link>
      </div>
      ...
      <div className="rounded-xl bg-kora-ink px-6 py-5 flex items-start justify-between">
        ...
        <div className="flex flex-col items-end gap-1.5 mt-1">
          <span className="rounded border border-kora-accent/60 bg-kora-accent/15 px-2 py-0.5 text-xs font-semibold text-white">COMPANY_ADMIN</span>
          <span className="text-xs text-white/25 font-mono">{userEmail} (admin preview)</span>
        </div>
      </div>

      {/* ── Calibration bar ────────────────────────────────────────────────── */}
      <div className="rounded-lg border border-[rgba(217,154,43,0.25)] bg-[rgba(217,154,43,0.08)] px-4 py-2.5 text-[10.5px] text-kora-warning-text font-medium">
          ...
          <div className="flex items-center gap-6 flex-wrap">
            <div>
              ...
              <div className="flex items-baseline gap-1.5">
                <span className="text-4xl font-bold text-kora-ink tracking-tight">{ki.value}</span>
            ...
            {Object.entries(ACME_MACROBLOCKS).map(([, mb]) => (
              <div key={mb.label} className="rounded border border-[rgba(6,3,43,0.08)] bg-[rgba(6,3,43,0.03)] px-3 py-2">
                ...
                <div className="mt-1 h-1.5 rounded-full bg-[rgba(6,3,43,0.12)]">
                  <div className="h-1.5 rounded-full bg-kora-accent" style={{ width: `${mb.score}%` }} />
                </div>
              </div>
            ))}
          </div>
      ...
      <Section title="Reporting Readiness" id="reporting-readiness">
        <div className="space-y-3">
          <div className="flex items-center gap-2 flex-wrap">
            <Badge label={rr.overallLevel.replace(/_/g, ' ')} cls="border-[rgba(217,154,43,0.25)] bg-[rgba(217,154,43,0.08)] text-kora-warning-text" />
          ...
          <div className="grid grid-cols-2 gap-2 text-[10px]">
            {rr.pillars.map((p) => (
              <div key={p.pillar} className="flex items-center gap-2">
                <span className={p.status === 'report_ready' ? 'text-kora-success' : p.status === 'not_ready' ? 'text-[rgba(158,59,47,0.75)]' : 'text-kora-warning'}>
      ...
      <Section title="Archivio Evidenze — Sola lettura" id="evidence-archive">
        <div className="space-y-3">
          <div className="flex gap-4 text-[10.5px] text-[rgba(6,3,43,0.52)]">
            ...
            <span><strong className="text-kora-warning">{ACME_EVIDENCE_SUMMARY.pendingReview}</strong> in revisione</span>
          </div>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            {ACME_SUBMISSIONS.map((sub) => (
              <div key={sub.id} className="rounded border border-[rgba(6,3,43,0.08)] bg-kora-paper px-3 py-2.5">
                <div className="flex items-center gap-2 flex-wrap">
                ...
                {sub.adminComment && sub.status !== 'submission_draft' && (
                  <p className="text-[9.5px] text-kora-warning-text mt-1.5 rounded border border-amber-100 bg-[rgba(217,154,43,0.08)] px-2 py-1">
      ...
      <Section title="Decision Pack" id="decision-pack">
        <div className="space-y-3">
          <div className="flex items-center gap-3 flex-wrap">
            <Badge label={dp.status} cls="border-[rgba(217,154,43,0.25)] bg-[rgba(217,154,43,0.08)] text-kora-warning-text" />
