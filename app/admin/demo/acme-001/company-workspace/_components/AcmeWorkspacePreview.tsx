...
const SUBMISSION_STATUS_CLS: Record<string, string> = {
  ...
  submission_draft:               'border-slate-200 bg-slate-50 text-slate-500',
};
    ...
    <div className="max-w-3xl mx-auto py-6 px-3 space-y-5">
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-6 flex-wrap">
            ...
            <div>
              ...
              <Badge
                ...
                cls={SAFEGUARD_CLS[ki.safeguardStatus] ?? 'border-slate-200 bg-slate-50 text-slate-500'}
              />
            </div>
            <div>
          ...
          <div className="grid grid-cols-2 gap-2 text-[10.5px]">
            {Object.entries(ACME_MACROBLOCKS).map(([, mb]) => (
              <div key={mb.label} className="rounded border border-slate-200 bg-slate-50 px-3 py-2">
                <div className="flex items-center justify-between">
                  <p className="font-semibold text-slate-700">{mb.label} ({Math.round(mb.weight * 100)}%)</p>
      ...
      <Section title="Reporting Readiness" id="reporting-readiness">
        <div className="space-y-3">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <span className="text-[10.5px] text-slate-500">{rr.pillars.filter(p => p.status === 'report_ready').length}/{rr.pillars.length} pillar pronti</span>
          </div>
          <div className="grid grid-cols-2 gap-2 text-[10px]">
            {rr.pillars.map((p) => (
              <div key={p.pillar} className="flex items-center gap-2">
                ...
                <span className={p.status === 'report_ready' ? 'text-slate-700' : 'text-slate-400'}>{p.pillar}</span>
              </div>
            ))}
          </div>
      ...
      <Section title="Archivio Evidenze — Sola lettura" id="evidence-archive">
        <div className="space-y-3">
          <div className="flex gap-4 text-[10.5px] text-slate-500">
            <span><strong className="text-slate-800">{ACME_EVIDENCE_SUMMARY.total}</strong> iniziative</span>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            {ACME_SUBMISSIONS.map((sub) => (
              <div key={sub.id} className="rounded border border-slate-200 bg-white px-3 py-2.5">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className="text-[10px] text-slate-500">{sub.submissionType}</span>
                  <span className="text-[9px] text-slate-400">{sub.period} · {sub.fileCount} file</span>
                </div>
      ...
      <Section title="Metodologia & Privacy" id="methodology">
        <div className="space-y-3 text-[10.5px] text-slate-600 leading-relaxed">
          ...
          <p className="text-[9.5px] text-slate-400">{ACME_METHODOLOGY.csrDisclaimer}</p>
        </div>
      </Section>
