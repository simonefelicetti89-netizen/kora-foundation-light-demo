...
const LIFECYCLE_CLS: Record<TenantLifecycleStatus, string> = {
  ...
  data_pending:           'bg-indigo-50 text-indigo-700 border-indigo-200',
  ...
  scoring_available:      'bg-violet-50 text-violet-700 border-violet-200',
  scored:                 'bg-emerald-50 text-emerald-700 border-emerald-200',
  decision_pack_available:'bg-green-50 text-green-800 border-green-300',
};
...
const DP_CLS: Record<string, string> = {
  ...
  exported: 'text-green-700 bg-green-50 border-green-200',
...
export function CompanyConsolePanel({ userEmail }: Props) {
  ...
  return (
    <div className="max-w-[1100px] mx-auto py-6 px-3 space-y-5">
        ...
        <div className="flex flex-col items-end gap-2 mt-1 shrink-0">
          ...
          <div className="flex gap-1 flex-wrap justify-end">
            {['LIVE PILOT', 'No PII', 'No Worker Data'].map(m => (
              <span key={m} className="rounded border border-white/15 bg-white/5 px-1.5 py-0.5 text-[9px] text-white/40 font-semibold uppercase">{m}</span>
            ))}
          </div>
        </div>
      </div>
      ...
      {/* ── Loading / Error ─────────────────────────────────────────────────── */}
      {loading && (
        <div className="rounded-lg border border-slate-100 bg-slate-50 px-4 py-8 text-xs text-slate-400 text-center">
          Caricamento Company Console…
        </div>
      )}
      {error && (
        <div className="rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-xs text-red-600">⚠ {error}</div>
      )}
        ...
        <>
          ...
          <div className="grid grid-cols-3 gap-3 sm:grid-cols-6">
            {[
              ...
              { label: 'Sospesi',             value: String(data.summary.suspended),        cls: data.summary.suspended > 0 ? 'text-slate-500' : undefined },
              { label: 'Scored',              value: String(data.summary.scored),           cls: 'text-violet-700' },
              { label: 'Decision Pack ready', value: String(data.summary.decisionPackReady), cls: 'text-emerald-700' },
              { label: 'Needs action',        value: String(data.summary.needsAction),      cls: data.summary.needsAction > 0 ? 'text-amber-700' : undefined },
            ].map(({ label, value, cls }) => (
          ...
          <div className="flex flex-wrap gap-2 items-center">
            <input
              ...
              className="rounded border border-slate-300 px-3 py-1.5 text-xs text-slate-800 w-48 focus:outline-none focus:ring-1 focus:ring-slate-400"
            />
            <select
          ...
          {data.tenants.length === 0 && (
            <div className="rounded-xl border border-slate-200 bg-white px-6 py-10 text-center space-y-3">
          ...
          {filtered.length === 0 && data.tenants.length > 0 && (
            <div className="rounded-lg border border-slate-200 bg-slate-50 px-4 py-4 text-xs text-slate-400 text-center">
  ...
  return (
    <tr className="border-b border-slate-50 hover:bg-slate-50/50">
      ...
      <td className="px-4 py-3 min-w-[160px]">
        <p className="font-semibold text-slate-800 leading-tight">{t.companyName}</p>
        <p className="text-slate-400 font-mono text-[9px] mt-0.5">{t.tenantCode}</p>
        <div className="flex gap-1 mt-1 flex-wrap">
          <Badge
            label={t.tenantStatus === 'active' ? 'ACTIVE' : 'SUSPENDED'}
            cls={t.tenantStatus === 'active' ? 'bg-green-50 text-green-700 border-green-200' : 'bg-slate-100 text-slate-500 border-slate-300'}
          />
          {t.warningFlags.length > 0 && (
            <Badge label={`${t.warningFlags.length} warn`} cls="bg-amber-50 text-amber-700 border-amber-200" />
          )}
        </div>
      </td>
      ...
      <td className="px-4 py-3 min-w-[100px]">
        {t.companyUsersCount === 0 ? (
          <span className="text-slate-400 italic">Nessun utente</span>
        ) : (
          <div>
            <p className="font-semibold text-slate-700">{t.companyUsersCount} utent{t.companyUsersCount === 1 ? 'e' : 'i'}</p>
            {t.primaryAdminEmail && (
              <p className="text-[9px] text-slate-400 mt-0.5 truncate max-w-[120px]" title={t.primaryAdminEmail}>
                {t.primaryAdminEmail}
              </p>
            )}
          </div>
      ...
      <td className="px-4 py-3 min-w-[80px]">
        {t.totalWorkers !== null ? (
          <p className="font-semibold text-slate-700">{t.totalWorkers.toLocaleString('it-IT')}</p>
        ) : (
        ...
        {t.latestKoraIndex ? (
          <div className="space-y-1">
            ...
            <p className="text-[9px] text-slate-500">
              CS: <strong className="text-slate-700">{Math.round(t.latestKoraIndex.confidenceScore * 100)}%</strong>
              <span className="text-slate-400"> · esterno</span>
            </p>
            <Badge
              label={t.latestKoraIndex.safeguardStatus}
              cls={SAFEGUARD_CLS[t.latestKoraIndex.safeguardStatus] ?? 'border-slate-200 bg-slate-50 text-slate-500'}
            />
            <p className="text-[8.5px] text-slate-400">{t.latestKoraIndex.reportingPeriod}</p>
          </div>
        ) : (
          <span className="text-slate-400 italic text-[10px]">Not scored yet</span>
        )}
      </td>
        ...
        {t.decisionPack ? (
          <div className="space-y-1">
            <Badge
              ...
              cls={DP_CLS[t.decisionPack.status] ?? 'border-slate-200 bg-slate-50 text-slate-500'}
            />
            <p className="text-[9px] text-slate-400">{ts(t.decisionPack.createdAt)}</p>
            <p className="text-[8.5px] text-slate-400">{t.decisionPack.versionId}</p>
          </div>
        ) : (
          <span className="text-slate-400 italic text-[10px]">Non generato</span>
        )}
      </td>
      ...
      <td className="px-4 py-3 min-w-[140px]">
        <div className="flex flex-col gap-1">
          ...
          <button
            onClick={copyId}
            className="text-[9px] text-slate-400 hover:text-slate-700 text-left transition-colors"
          >
