...
const INTAKE_STATUS_COLORS: Record<string, string> = {
  ...
  ready_for_ingestion:         'border-emerald-200 bg-emerald-50 text-emerald-700',
  blocked_missing_required_fields: 'border-rose-200 bg-rose-50 text-rose-700',
};
    ...
    <div className="space-y-8 max-w-6xl">
      ...
      {plan && plan.fiscal_perimeters.length > 0 ? (
                ...
                {fp.risk_flags.length > 0 && (
                  <div className="space-y-0.5">
                    {fp.risk_flags.map((rf, i) => (
                      <p key={i} className="text-[9px] text-amber-700 bg-amber-50 rounded px-2 py-1 leading-relaxed">{rf}</p>
                    ))}
                  </div>
                )}
        ...
        <section className="rounded-lg border border-amber-200 bg-amber-50 p-5 space-y-2">
          <p className="text-xs font-semibold text-amber-800">Perimetro fiscale non ancora definito</p>
              ...
              <tbody>
                {batches.map((b) => (
                  <tr key={b.batch_id} className="border-b border-[rgba(6,3,43,0.05)] last:border-0 hover:bg-[rgba(6,3,43,0.03)]">
                    ...
                    <td className="px-3 py-2 text-right font-mono text-rose-600">{b.blocked_candidate_count}</td>
                ...
                {visibleRows.map((row) => (
                  <tr key={row.row_id} className={cn('border-b border-[rgba(6,3,43,0.05)] last:border-0 hover:bg-[rgba(6,3,43,0.03)]', !row.ready_for_ingestion && 'opacity-70')}>
                    ...
                    <td className="px-3 py-2 text-center">
                      <span className={cn('font-bold text-sm', row.ready_for_ingestion ? 'text-emerald-600' : 'text-rose-500')}>
      ...
      <section className="space-y-3">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
          {[
            { label: 'Eligible candidates', count: eligible.length, color: 'border-emerald-200 bg-emerald-50', textColor: 'text-emerald-700', note: 'Potenzialmente eligible per IU' },
            { label: 'Limited candidates', count: limited.length, color: 'border-amber-200 bg-amber-50', textColor: 'text-amber-700', note: 'BTI only — 0 IU per design' },
            { label: 'Blocked candidates', count: blocked.length, color: 'border-rose-200 bg-rose-50', textColor: 'text-rose-700', note: '0 IU · 0 KORA Index · governance only' },
              ...
              <div key={row.row_id} className="px-4 py-3 space-y-1">
                ...
                <div className="flex flex-wrap gap-1">
                  {row.missing_fields.map((f) => (
                    <span key={f} className="rounded bg-rose-50 border border-rose-200 px-1.5 py-0.5 text-[9px] text-rose-700 font-mono">{f}</span>
                  ))}
                </div>
