...
describe('DEMO-GUARD-01 — canonical Decision Pack authority has zero dependency on legacy report chain', () => {
  ...
  it('ReportGeneratorService retired (B-TRUTH ReportGenerator Retirement, 2026-09-02) — the rest of the legacy chain is untouched, one-PR-one-service discipline', () => {
    ...
    expect(fs.existsSync(path.join(ROOT, 'services/financial-governance/FinancialGovernanceService.ts'))).toBe(true);
  });
});
