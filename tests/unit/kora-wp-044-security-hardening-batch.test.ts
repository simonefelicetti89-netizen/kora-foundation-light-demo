...
describe('KORA-WP-044 — nessuna migrazione, nessun nuovo modulo di sicurezza generico introdotto', () => {
  it('supabase/migrations/ resta invariato alla ceiling 080 (Data/Migration Impact: NONE, registry 142)', async () => {
    // Ceiling bumped 80→...→88 by later, unrelated WPs/increments
    // (including KORAL Morphology Package A's own migrations 087 and 088,
    // 2026-09-19) — this assertion's own intent ("WP-044 itself adds no
    // migration") is unaffected; only the global ceiling this test pins
    // to has moved.
