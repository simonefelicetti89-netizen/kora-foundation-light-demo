...
> **Master Plan precedence (effective 2026-08-30):** `docs/KORA_OFFICIAL_IMPLEMENTATION_MASTER_PLAN_v2.1.md` ("KORA — OFFICIAL IMPLEMENTATION MASTER PLAN v2.1", FINAL FREEZE VERSION) is the current architecture execution authority for domain ontology (Program/Investment/Decision/Survey/Needs layer), the Architecture Registry, the constitutional invariants, consolidation/new-block sequencing, the dependency graph, the Claude Code execution protocol, and the Definition of Done. v2.1 supersedes `docs/KORA_OFFICIAL_IMPLEMENTATION_MASTER_PLAN_v2.0.md` as execution authority; v2.0 remains available as historical baseline and must not be deleted, renamed, or modified. **The repository is the operational truth.** Where the Master Plan and the repository disagree, do not silently pick one or reconcile on your own judgment — follow the Master Plan's own Read-Before-Write protocol, report `STATE_MATCH = NO`, describe the mismatch, and stop for human review. No task may silently reduce the commercial promise defined in the Master Plan to match the current state of the code. This precedence does not touch product positioning, the five pillars, KORA Index v3 component definitions, the 14-stage algorithm, the IU formula, Italian-first language policy, or privacy rules below — those remain governed as stated in this hierarchy.

> **Execution Registry precedence (effective 2026-09-12, closing red-team finding RT-P1-001):** `.kora-audit/output/102_KORA_MASTER_PLAN_FINAL_CANONICAL_EXECUTION_REGISTRY_VERIFIED.md` is the **sole canonical executable registry** for `KORA-WP-001` through `KORA-WP-110`: implementation scope, WP acceptance boundaries, WP sequencing, hard dependencies, conditional dependencies, scope triggers, implementation milestones, and Base Pilot blocker status are all governed by `102`, not by `v2.1` or by any earlier `.kora-audit/output` registry (`96`, `99`, and earlier remain historical provenance only). `docs/KORA_OFFICIAL_IMPLEMENTATION_MASTER_PLAN_v2.1.md` remains a valid architecture/planning reference and historical implementation-plan source — it does **not** govern current executable WP sequencing now that `102` exists, and must not be used by a future session to override `102`'s ordering or scope. Neither direction runs the other way either: `102` governs execution scope/sequencing only — it does not override frozen Product Truth, Commercial Truth, Economic Truth, Code Truth, or DD-1/2/3/4 domain semantics; when implementing a WP, use `102` for scope/sequencing and that WP's own cited frozen sources for domain semantics. If executing a WP surfaces an apparent contradiction between `102` and a higher-authority frozen source, do not silently pick one or reinterpret either — stop and escalate for Founder review, the same as any other `STATE_MATCH = NO` case above.
>
> **Forward execution gate — `KORA-WP-069`:** per independent audit finding RT-P1-002 (`.kora-audit/output/104_KORA_INDEPENDENT_PREIMPLEMENTATION_RED_TEAM_AUDIT.md`), `KORA-WP-069` must not start until external legal blocker **F-12** (pseudonymous vs. anonymous handling of individual worker Listening responses) is explicitly resolved. This gate does not block `KORA-WP-001` or any other package in the current execution wave. Before implementing `KORA-WP-069`, the implementing agent must verify F-12's closure status; if F-12 remains open, `KORA-WP-069`'s status is `BLOCKED — EXTERNAL LEGAL GATE`. F-12 is not resolved by this note, and its substance is not defined here beyond what `104` and its cited source already establish.
...
2. **`docs/KORA_OFFICIAL_IMPLEMENTATION_MASTER_PLAN_v2.1.md`** (FINAL FREEZE VERSION) — target architecture, domain ontology, Architecture Registry, constitutional invariants, execution governance, and the Claude Code protocol described above. Supersedes `docs/KORA_OFFICIAL_IMPLEMENTATION_MASTER_PLAN_v2.0.md`, which remains as historical baseline.
...
2. `docs/KORA_OFFICIAL_IMPLEMENTATION_MASTER_PLAN_v2.1.md` — architectural truth for target architecture, domain ontology, and execution governance (supersedes v2.0, which remains historical baseline)
