...
export function PipelineConnectorBanner() {
  ...
  PIPELINE_STEPS.forEach((step, i) => {
    pipelineElements.push(
      <Link
        key={`step-${step.num}`}
        href={step.href}
        className="flex-1 rounded-lg border border-[rgba(199,111,61,0.22)] bg-[#F8F6F1] p-3 text-xs hover:border-[#C76F3D] hover:shadow-sm transition-all min-w-0"
      >
        <p className="font-mono text-[10px] font-bold text-[rgba(6,3,43,0.30)] mb-1">
    ...
    if (i < PIPELINE_STEPS.length - 1) {
      pipelineElements.push(
        <div
          key={`arrow-${i}`}
          className="hidden sm:flex items-center justify-center shrink-0 px-1.5 text-[rgba(6,3,43,0.20)] text-sm select-none"
        >
          →
        </div>,
      );
  ...
  return (
    <div className="rounded-lg border border-[rgba(6,3,43,0.06)] bg-[rgba(199,111,61,0.06)] p-5 space-y-4">
      <div>
        <h2 className="text-sm font-semibold text-[#06032B]">
          Flusso operativo KORA Operator
        </h2>
        <p className="mt-1 text-xs text-[#C76F3D] leading-relaxed max-w-2xl">
        ...
        {scenario && (
          <p className="mt-1.5 text-xs font-medium text-[#C76F3D]">
      ...
      <p className="text-[10px] text-[rgba(6,3,43,0.40)] leading-relaxed">
