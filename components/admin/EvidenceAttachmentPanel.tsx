...
const LEVEL_COLORS: Record<string, string> = {
  ...
  L2: 'text-kora-accent bg-kora-accent/8 border-kora-accent/25',
  L1: 'text-kora-warning-text bg-[rgba(217,154,43,0.08)] border-[rgba(217,154,43,0.25)]',
  L0: 'text-[rgba(6,3,43,0.52)] bg-[rgba(6,3,43,0.03)] border-[rgba(6,3,43,0.08)]',
};

const STATUS_COLORS: Record<string, string> = {
  parsed_metadata: 'text-green-700',
  metadata_only:   'text-kora-warning-text',
  needs_review:    'text-kora-warning-text',
  rejected_pii:    'text-kora-critical',
  rejected_size:   'text-kora-critical',
...
export function EvidenceAttachmentPanel({ tenantCode, batchId }: { tenantCode: string; batchId: string }) {
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-lg border border-kora-accent/25 bg-kora-accent/8 px-4 py-3">
        <p className="text-[10px] font-bold text-kora-accent uppercase tracking-wide mb-1">Evidence Attachment — B34 Private Storage</p>
        <p className="text-[10px] text-kora-ink-secondary leading-relaxed">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div>
          ...
          <input type="file" accept=".pdf,.xlsx,.csv" aria-label="Documento"
            ...
            className="text-xs text-[rgba(6,3,43,0.62)] file:mr-2 file:py-1 file:px-2 file:rounded file:border file:border-[rgba(6,3,43,0.08)] file:bg-kora-paper file:text-xs file:cursor-pointer" />
      ...
      {preview?.ok && preview.metadata && (
        <div className="rounded-lg border border-[rgba(6,3,43,0.08)] bg-kora-paper px-4 py-3 space-y-2">
          ...
          <p className="text-[10px] text-kora-ink-secondary bg-kora-accent/8 border border-kora-accent/25 rounded px-2 py-1">
      ...
      {previewStatus === 'done' && preview?.ok && preview.metadata?.parserStatus !== 'rejected_pii' && preview.metadata?.parserStatus !== 'rejected_size' && registerStatus !== 'done' && (
        <div className="space-y-2">
          ...
          {!batchId && <p className="text-[10px] text-kora-critical">⚠ batchId non disponibile — seleziona un batch valido.</p>}
        </div>
      )}
      ...
      {registerStatus === 'done' && registerResult?.ok && (
        <div className="rounded border border-[rgba(47,125,85,0.22)] bg-green-50 px-3 py-2 text-[10px] text-green-700 space-y-1.5">
          ...
          <div className="flex flex-wrap gap-1.5">
            ...
            <span className="rounded border border-[rgba(47,125,85,0.22)] bg-kora-paper px-1.5 py-0.5 text-[9px] font-bold">
          ...
          {registerResult.storageStatus === 'stored_private' && registerResult.attachmentId && (
            <div className="pt-1 border-t border-green-100">
              <button
                ...
                className="rounded border border-green-300 bg-kora-paper px-3 py-1 text-[10px] font-semibold text-kora-success hover:bg-green-50 disabled:opacity-50 transition-colors"
              >
                {openLinkStatus === 'loading' ? '⏳ Generazione link…' : '🔒 Apri documento sicuro'}
              </button>
              <p className="text-[9px] text-kora-warning-text mt-0.5">Link temporaneo (5 min). Non condividere.</p>
              {openLinkStatus === 'error' && <p className="text-[9px] text-kora-critical">⚠ {openLinkError}</p>}
            </div>
          )}
