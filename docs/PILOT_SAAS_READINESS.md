...
- **RLS negative/positive testing is nearly complete at the DB level, still open at the authenticated-request level.** Tenant-vs-tenant (RLS-03) and worker-vs-worker (RLS-05) negative isolation are both DB-level proven live. App/API-level source code is statically audited (RLS-04). KORA_ADMIN's legitimate cross-tenant/admin access is confirmed statically (RLS-06, 2026-07-04 — see `docs/QA_STATUS.md`), with a live direct-Postgres control test implemented skip-safe but not yet run. None of the four has a live authenticated-request/PostgREST proof.
...
5. **RLS-06 live run** — RLS-03 and RLS-05 (DB-level negative isolation) are both merged and were both run live. RLS-06 (KORA_ADMIN legitimate-access positive control, closing the "false regression trap" risk of a negative-only suite) landed 2026-07-04 with its static/unit portion fully verified (`tests/unit/rls06-kora-admin-access-control.test.ts`, 76 assertions) and its live direct-Postgres portion (`tests/integration/rls-kora-admin-control.test.ts`) implemented skip-safe but not yet run against a live local Supabase instance — do that next. A live/authenticated-request E2E proof of the RLS-04 app/API boundary also remains open (see `docs/QA_STATUS.md`).
