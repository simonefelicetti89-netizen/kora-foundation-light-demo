...
- **RLS negative testing is partial** — tenant-vs-tenant DB-level proven live (RLS-03), app/API-level source code audited and statically enforced (RLS-04), worker-vs-worker DB-level implemented skip-safe but **not yet run live** (RLS-05, 2026-07-04 — see `docs/QA_STATUS.md`). None of the three has a live authenticated-request/PostgREST proof.
...
5. **RLS-05 live run, then RLS-06** — RLS-05 (worker-vs-worker DB isolation on `personal.worker_identity`/`worker_pib`) is implemented and skip-safe-verified as of 2026-07-04 (`tests/integration/rls-worker-isolation.test.ts`) but has not yet been run against a live local Supabase instance — do that first. RLS-06 (a control test confirming KORA_ADMIN's legitimate cross-tenant access still works, so the negative tests don't become a false regression trap) remains open after. A live/authenticated-request E2E proof of the RLS-04 app/API boundary also remains open (see `docs/QA_STATUS.md`).
