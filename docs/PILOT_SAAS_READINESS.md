...
- **A golden path E2E for the data-bearing steps is implemented but not yet run live.** `tests/e2e/golden-data-bearing.spec.ts` (GOLDEN-E2E-02, 2026-07-04) drives upload → UEF generate/approve → scoring → Decision Pack → COMPANY_ADMIN visibility through the real `/admin/data-intake` and `/admin/uef-review` UI (no mocks) — skip-safe-verified locally with no credentials, gated by an explicit `E2E_GOLDEN_DATA_BEARING_ALLOW_RUN=true` (mutates real tenant data, unlike every other E2E file here). Not yet executed against staging with real credentials. `tests/unit/b103-golden-path.test.ts` checks file existence, not behavior — don't cite it as coverage. `tests/e2e/golden-admin-company.spec.ts` (GOLDEN-E2E-01) automates the admin/company workspace + data-surface-reachability half, plus a markup-level privacy smoke check. **Manual/staging authenticated tests are deferred until the final pilot-validation session. GD01 exists as a skip-safe E2E and remains implemented-but-not-live-proven until that session.**
...
3. Build at least one automated golden-path E2E (upload → scoring → Decision Pack) — **implemented, not yet run live:** `golden-admin-company.spec.ts` (GOLDEN-E2E-01) automates admin/company workspace reachability + a real data/report surface + a privacy smoke check; `golden-data-bearing.spec.ts` (GOLDEN-E2E-02) now automates the upload → UEF → scoring → Decision Pack chain itself through the real UI, skip-safe-verified locally but not yet executed against staging with real credentials.
