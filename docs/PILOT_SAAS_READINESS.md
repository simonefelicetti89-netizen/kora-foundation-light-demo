...
- **RLS negative testing is partial** — DB-level proven (RLS-03), app/API-level source code audited and statically enforced (RLS-04, 2026-07-04 — see `docs/QA_STATUS.md`), but neither has a live/authenticated-request proof, and worker-vs-worker (RLS-05) is still open.
...
5. **RLS-05/06** — worker-vs-worker isolation and a KORA_ADMIN legitimate-cross-tenant-access control test. RLS-04 (app/API-level static enforcement) is done as of 2026-07-04 (see `docs/QA_STATUS.md`); a live/authenticated-request E2E proof of the same boundary remains open.
