...
- **A golden path E2E for the data-bearing steps is implemented, and repo evidence indicates it has since been run live.** `tests/e2e/golden-data-bearing.spec.ts` (GOLDEN-E2E-02, 2026-07-04) drives upload → UEF generate/approve → scoring → Decision Pack → COMPANY_ADMIN visibility through the real `/admin/data-intake` and `/admin/uef-review` UI (no mocks) — skip-safe-verified locally with no credentials, gated by an explicit `E2E_GOLDEN_DATA_BEARING_ALLOW_RUN=true` (mutates real tenant data, unlike every other E2E file here). **Updated by B174-A2 (2026-07-12):** this bullet previously said `GD01` had not yet been executed against staging. Repo evidence (`docs/E2E_GOLDEN_PATH.md`) indicates it was run live against staging, with explicit founder approval, on **2026-07-09**, and passed. This has **not been independently re-verified in a later session** — a fresh, operator-approved confirmation is recommended before this is cited in a client-facing claim. `tests/unit/b103-golden-path.test.ts` checks file existence, not behavior — don't cite it as coverage. `tests/e2e/golden-admin-company.spec.ts` (GOLDEN-E2E-01) automates the admin/company workspace + data-surface-reachability half, plus a markup-level privacy smoke check, and remains not run via fixture as of this writing.
- **RLS negative/positive testing is mostly complete at the DB level, still open at the authenticated-request level.** Tenant-vs-tenant (RLS-03) and worker-vs-worker (RLS-05) isolation are both DB-level proven live. App/API-level source code is statically audited (RLS-04). KORA_ADMIN's legitimate cross-tenant/admin access (RLS-06) is proven statically (76 assertions); its live direct-Postgres control test is implemented skip-safe but **has not yet been run** — there is no "RLS-06B" live-run milestone yet, despite a prior version of this line claiming one; see `QA_STATUS.md`'s RLS-06 entry for the accurate status. None of the four has a live authenticated-request/PostgREST proof.
- **COMPANY_B status (updated by B174-A2, 2026-07-12):** this bullet previously stated "COMPANY_B doesn't exist," blocking any real two-tenant demonstration. That was true as a blocker **before 2026-07-09**. Repo evidence (`docs/E2E_TWO_TENANT_ISOLATION.md`) indicates COMPANY_B was provisioned in staging and live-validated (`A02`–`A04`, `T01`, `T02`) on that date. This has **not been independently re-verified since** — treat current status as "repo evidence indicates resolved," not as a freshly-confirmed fact, and obtain a read-only or operator-approved confirmation before relying on it for any further mutation or client-facing claim.
...
- A second company tenant for side-by-side demonstration as a currently-live-verified fact — repo evidence indicates COMPANY_B was provisioned and validated in staging on 2026-07-09 (see the COMPANY_B status note above), but this has not been independently re-verified since, so it should not be promised as a freshly-confirmed capability without a fresh confirmation first.
...
1. ~~Provision a real COMPANY_B (or the pilot client's actual second reference tenant) — currently blocks any live two-tenant demonstration.~~ **Updated by B174-A2 (2026-07-12):** repo evidence indicates this was done in staging on 2026-07-09 (see `docs/E2E_TWO_TENANT_ISOLATION.md`). Not independently re-verified since — a fresh confirmation is recommended before this is treated as a closed item for client-facing purposes.
2. ~~Run `A02` (COMPANY_A E2E) locally and in Production — fixture exists, unexecuted.~~ **Updated by B174-A2:** documented as run against staging on 2026-07-09 (passed). Not yet run specifically against Production; not independently re-verified since.
3. Build at least one automated golden-path E2E (upload → scoring → Decision Pack) — **implemented; `golden-data-bearing.spec.ts` (GOLDEN-E2E-02, test `GD01`) is documented as run live against staging on 2026-07-09 (passed) per `docs/E2E_GOLDEN_PATH.md`, not independently re-verified since.** `golden-admin-company.spec.ts` (GOLDEN-E2E-01) automates admin/company workspace reachability + a real data/report surface + a privacy smoke check and remains not run via fixture as of this writing.
