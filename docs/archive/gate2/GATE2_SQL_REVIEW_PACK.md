...
| 011 | `011_worker_cv_share` | personal | `personal.worker_cv_share` | Yes, FORCE RLS | No | HIGH (share token, kora_worker_id in JWT) | NEEDS_LEGAL_PRIVACY_REVIEW | CTO + DPO | **Role reads updated 2026-06-21** — `kora.kora_role()` now used for KORA_ADMIN and WORKER role checks. `kora_worker_id` raw read intentionally retained (no canonical helper — see Gate 2 CTO design question §7 Q10). Token is hashed; share-link UX requires DPO review. |
...
- [x] **Migration 011 role reads updated to canonical helper.** ~~Raw `(auth.jwt() -> 'app_metadata' ->> 'kora_role')` in mig 011.~~ **Fixed 2026-06-21** — KORA_ADMIN and WORKER role checks now use `kora.kora_role()`. Raw `kora_worker_id` read intentionally retained (no canonical helper — Gate 2 design question §7 Q10).
- [ ] **`kora_worker_id` JWT field provisioning documented + canonical helper decision made.** Migration 011 reads `app_metadata.kora_worker_id` (raw) to authorize worker CV share access. Provisioning path: `app/api/admin/workers/provision/route.ts` writes `kora_worker_id` to `app_metadata` via `auth-admin-update-user`. The raw read is intentionally retained because no `kora.worker_id()` canonical helper exists. CTO must decide: introduce `kora.worker_id()` or accept raw read as documented exception. See §7 Q10.
...
2. ~~**`kora_worker_id` provisioning path undocumented.**~~ **Partially resolved 2026-06-21** — `app/api/admin/workers/provision/route.ts` step 4 writes `kora_worker_id` to `app_metadata` via `lib/supabase/auth-admin-update-user.ts`. Provisioning path is now documented. Remaining question → see Q10.
...
11. **★ `kora.worker_id()` canonical helper — CTO design decision required.** Migration 011 (`worker_cv_share`) uses `(auth.jwt() -> 'app_metadata' ->> 'kora_worker_id')::uuid` to scope worker access to their own CV share rows. This is the only migration that reads `kora_worker_id`. Two canonical helpers exist: `kora.kora_role()` and `kora.tenant_id()`. No `kora.worker_id()` helper exists. Options: (a) introduce `kora.worker_id()` returning uuid — consistent with existing helper pattern, centralises the claim key name, allows future migration; (b) accept the raw read as a documented exception — justified because worker identity is the primary access key, not a scope filter, and it only appears in one migration. CTO must decide before migration 011 is reviewed as complete. The raw read is currently retained with an explanatory comment in the migration file.
