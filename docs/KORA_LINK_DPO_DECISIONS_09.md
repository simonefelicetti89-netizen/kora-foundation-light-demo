...
## Testo del notice (proposta — italiano, da approvare)
...
**10. Versione canonica identificabile:** v. BLOCCO 4 sotto.

**Nota:** questo testo è una **proposta**, non ancora un testo DPO-approvato. Deve essere confermato esplicitamente dal titolare (eventualmente con supporto legale) prima di essere inserito come valore reale del whitelist `consent_version` in `036`.
...
2. **`036_kora_link_rpc_functions.sql`**: `c_valid_consent_version` in `fn_activate_link_for_worker` aggiornato al valore canonico approvato `kora-link-activation-notice-v1.0` (DPO-05); gli `INSERT`/`UPDATE` della funzione ora scrivono su `kora_link.link_activation_acknowledgements` con le colonne rinominate. Il parametro RPC esterno `p_consent_version` **non è stato rinominato** — solo il valore e la tabella/colonne interne.
3. **`lib/kora-link/activation.ts`**: `KORA_LINK_ACTIVATION_CONSENT_VERSION` aggiornato al valore canonico approvato, identico a `036` (DPO-05); rimosso il commento "provisional — final copy/version requires DPO/legal approval". Il nome della costante esportata **non è stato rinominato**.
