...
4. Runs the RLS integration suites (RLS-03/05/06/07/08/09/10/11/12/13) against that real local Postgres, with an explicit machine-readable assertion that exactly 0 were skipped and 0 failed — a suite silently not running (e.g. a typo'd env var) is a hard failure, not a quiet pass. RLS-11 (CC-052, 2026-08-31) proves commons.post's cross-tenant WORKER visibility (migration 024's cross_company policy) behaves as designed — a worker discovers a cross_company published post from another tenant, but not a company_internal one. RLS-12 (synthetic-company foundation, 2026-08-31) proves tenant_kind never participates in product-level RLS/business logic — a DEMO-kind tenant's COMPANY_ADMIN inserts/selects commons.post through the identical policies a LIVE-kind tenant uses. RLS-13 (B-TRUTH first canonical seed group, 2026-09-01) proves personal.workforce_baseline — the live source that replaced WorkforceBaselineService's synthetic JSON — returns identical data for a DEMO-kind and a LIVE-kind tenant.
