...
4. Runs the RLS integration suites (RLS-03/05/06/07/08/09/10/11/12/13/14/15/16) against that real local Postgres, with an explicit machine-readable assertion that exactly 0 were skipped and 0 failed — a suite silently not running (e.g. a typo'd env var) is a hard failure, not a quiet pass. RLS-11 (CC-052, 2026-08-31) proves commons.post's cross-tenant WORKER visibility (migration 024's cross_company policy) behaves as designed — a worker discovers a cross_company published post from another tenant, but not a company_internal one. RLS-12 (synthetic-company foundation, 2026-08-31) proves tenant_kind never participates in product-level RLS/business logic — a DEMO-kind tenant's COMPANY_ADMIN inserts/selects commons.post through the identical policies a LIVE-kind tenant uses. RLS-13 (B-TRUTH first canonical seed group, 2026-09-01) proves personal.workforce_baseline — the live source that replaced WorkforceBaselineService's synthetic JSON — returns identical data for a DEMO-kind and a LIVE-kind tenant. RLS-14 (B-TRUTH Contribution protected port, 2026-09-01) proves commons.contribution_event + commons.post — the live source that replaced KoraContributionService.getSummaryV2's synthetic JSON input — feed the unchanged computeContributionV2/computeFromPipelineResult methodology authority to an identical ContributionSummary.v2 output for a DEMO-kind and a LIVE-kind tenant. RLS-15 (B-TRUTH Company Onboarding canonicalization, 2026-09-01) proves analytics.tenant + personal.workforce_baseline — the live source that replaced CompanyOnboardingService's synthetic JSON — feed buildCompanyOnboardingView to an identical onboarding view (Foundation Light eligibility, pipeline readiness, next-best-action) for a DEMO-kind and a LIVE-kind tenant. RLS-16 (B-TRUTH Ingestion/UEF PR1, 2026-09-02) proves a canonical synthetic/example (DEMO-kind) tenant ingests deterministic fake data through the exact same Ingestion/UEF runtime (analytics.source_batch, personal.uploaded_record, analytics.uef_record, classifyEligibilityBatch, buildScoringRecordsFromApprovedUef) and the exact same runKoraPipeline authority a LIVE-kind tenant uses, with matching negative tenant isolation — "fake data, real company model, real KORA engine."
