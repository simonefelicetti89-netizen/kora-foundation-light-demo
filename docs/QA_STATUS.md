...
- **RLS negative testing exists but only at the direct-Postgres/local level.** See "RLS negative testing, local direct-Postgres" above — merged to `main` via PR #26. It closes the original GOLDEN-01 gap for DB-level tenant isolation on the four tables it covers, but does not cover `personal.*` (worker-vs-worker, RLS-05) and does not run through PostgREST or GoTrue. Treat "RLS negative testing" as partially, not fully, closed until RLS-05/RLS-06 land.
...
5. RLS negative tests — **partially done:** cross-tenant read denial now proven at the direct-Postgres/local level for `analytics.source_batch`/`kora_index_result`/`activation_result` (see above), merged to `main`. App/API-level static enforcement mapped and covered (RLS-04, see above) — no live/authenticated request-level proof yet. Still open: worker-vs-worker isolation (RLS-05); a control test confirming KORA_ADMIN's legitimate cross-tenant access still works (RLS-06), so the negative tests don't become a false regression trap; an authenticated E2E extension proving cross-tenant/cross-worker requests actually get rejected at runtime (see RLS-04 entry above for the priority route list).
