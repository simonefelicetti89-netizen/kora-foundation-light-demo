...
**RLS negative testing, local direct-Postgres (RLS-03, branch `test/rls03-direct-postgres`, not yet merged to `main`):**
...
- **RLS negative testing exists but only at the direct-Postgres/local level, and only on an unmerged branch.** See "RLS negative testing, local direct-Postgres" above. It closes the original GOLDEN-01 gap for DB-level tenant isolation on the four tables it covers, but does not cover `personal.*` (worker-vs-worker, RLS-05), does not run through PostgREST or GoTrue (RLS-04's job), and has not yet been merged to `main`. Treat "RLS negative testing" as partially, not fully, closed until RLS-04/05/06 land and this branch merges.
