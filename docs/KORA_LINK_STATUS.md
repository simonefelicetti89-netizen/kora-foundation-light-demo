...
| Gate 4 — RLS 035 Review | Open — **draft exists** (`supabase/proposed/035_kora_link_rls.sql`, RLS policies: KORA_ADMIN-only across all 9 tables), but incomplete: the worker-self-select policy on `link_assignments` remains commented out by design. No company-facing SELECT policy exists at all yet (aggregate-only via `fn_company_link_status_aggregate` in `036`). Not reviewed/applied. **KORA-LINK-S3A (2026-07-12, draft-only, engineering-only):** closed a real grant-hygiene gap — added explicit `service_role` grants (schema/tables/functions) matching the same bug shape migrations 032/033 had to retroactively fix, added `REVOKE ALL FROM PUBLIC` before `is_kora_admin()`'s grant for consistency, removed 035's now-obsolete `fn_public_lookup_link`/`fn_activate_link_for_worker` stub SQL (both are real, implemented, uncommented functions in `036` — the design-rationale prose was kept, only the dead stub code was removed), and added `tests/unit/kora-link-rls035-review.test.ts` (93 static assertions). **This does not close Gate 4** — worker self-select and company-facing SELECT remain exactly as open as before. |
