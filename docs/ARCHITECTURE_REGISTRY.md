...
| `svc.report-factory` | Reporting | `services/report-factory/ReportFactoryService.ts` | CONSOLIDATE | — | CC-013 / D-B | D-B resolved (CC-005/CC-013): lib.decision-pack is canonical. ReportFactoryService is NOT canonical — retained as the admin-side metadata/status/comparison helper for app/admin/pipeline and app/admin/companies/[companyId] (2 real callers), but its data is 100% synthetic (imports data/synthetic/decision-pack-versions.json directly, zero DB queries) — a live admin page displaying synthetic version history is a real One-Truth divergence, consolidation pending B-TRUTH, not fixed in CC-013. |
