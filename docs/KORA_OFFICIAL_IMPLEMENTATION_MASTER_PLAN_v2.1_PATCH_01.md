...
**Action on `lib/architecture/registry.ts`:** none required on `main` — the registry already correctly shows `svc.company-intelligence` and `svc.company-onboarding` as `INVESTIGATE`, unaffected by the not-yet-merged PR #113. A clarifying note is added (§7 below) recording this patch's finding for future reference.
...
Only **CC-020/CC-020A** (`CompanyIntelligenceService`/`CompanyOnboardingService` physical deletion, PR #113, not merged) is held pending §5 above. Nothing else already on `main` is reopened, reversed, or requires re-verification beyond what this patch already performed.
...
- `svc.company-intelligence`, `svc.company-onboarding`: added a note referencing this patch's §5 finding and PR #113's held (not merged) status.
...
The one file requiring attention, `tests/unit/cc020a-retire-company-intelligence.test.ts`, exists **only on the unmerged PR #113 branch** — it is not present on `main` and therefore requires no action here. Its assertions currently encode the held deletion as a passing state; per §5, it must not be merged as-is. When `CompanyIntelligenceService`'s disposition is eventually resolved (either via a §32 addendum or a permanent `INVESTIGATE`), that test file's assertions need to match whichever outcome is chosen — this is deferred to that future task, not performed here.

---

## 9. Next official B-TRUTH action

The task that prompted this patch proposed `ScoringSimulatorService + DemoScoringAdapter + kora-index-outputs.json` as the next candidate. **Independently verified and disqualified**, not merely accepted: §32's own Safe Deletion Plan table names this exact group — `DemoScoringAdapter · ScoringSimulatorService · demo-data · access-control` — with the explicit condition `fine B-TRUTH` (end of B-TRUTH) and `I9 = 0, adversarial superata` (I9 already zero, adversarial already passed). This is the frozen text's own designated **last** group, not a viable next step while I9 is still 24 (or 23 pending PR #113) and CC-022/023 have not started. Migrating it now would violate §32's own explicit sequencing.

`NEXT_OFFICIAL_B_TRUTH_TARGET = NONE_SAFELY_AVAILABLE` under the current, corrected governance. A prior exhaustive elimination pass over the full I9 allowlist (conducted before CC-019 was authorized) already found every remaining candidate outside the TenantService cluster to be one of: explicitly reserved (`ScoringSimulatorService`/`DemoDataService`/`CommonsService`/`KoraContributionService`/the Ingestion/UEF `DEMO_RUNTIME` chain), already demo-labeled (`WorkforceBaselineService`, `WorkforceQuickAccessPanel`), zero-live-reachable (`CompanyOnboardingService` pre-CC-020A, `CompanyDataIntakeService`, `FinancialGovernanceService`), or gated by the separate, unresolved D-D/B-WORKER decision (`WorkerAchievementService`, `/my-kora`-facing consumers). With CC-020 now held, no new candidate has opened up. Unblocking a genuine next seed-group migration requires either resolving §5's `CompanyIntelligenceService` question through the proper mechanism, or a founder decision to pull a currently-reserved or D-D-gated group forward explicitly.
