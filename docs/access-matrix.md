...
- **`ADVISOR` (*):** dal ROLE-01, `canAccess()` restituisce `DENY` esplicito per `ADVISOR` su ogni risorsa di questa tabella — non è più un caso non modellato. Per `worker_individual_uef` specificamente, questo `DENY` riguarda l'accesso diretto/individuale via questa risorsa app-layer; `analytics.fn_advisor_uef_read()` esiste separatamente a livello DB (migration 030, `SECURITY DEFINER`, tenant-scoped, payload escluso) ma **nessuna route la richiama oggi**. **Aggiornamento (KORA-WP-002, 2026-09-12):** `requireAdvisorUser()` / `isAdvisorUser()` / `getCurrentAdvisorUser()` ora esistono in `lib/auth/kora-session.ts` — il guard di sessione riconosce un `kora_role: 'ADVISOR'` valido, e `middleware.ts` blocca una sessione ADVISOR da `/worker/*` (stesso layer di difesa già usato per `KORA_ADMIN`, B168-P3). Non ancora esistenti: la tabella Advisor Identity (`KORA-WP-030`), il login/provisioning ADVISOR, e una route reale — `/advisor` reindirizza ancora permanentemente a `/demo/advisor` (showcase statico), e `ADVISOR` resta fuori da `ACTIVE_KORA_ROLES`. Non promettere accesso advisor reale ai clienti. Vedi `docs/FUTURE_ROLES_AND_SURFACES.md`.
...
| `company_living_koral` | KORA-WP-114 — Living KORAL Company Hub (`/company/living-koral`), read-only, own tenant only — current structural state + latest recognized transformation, mirrors `company_kpi_kora_index`'s exact shape (KORA_ADMIN: `allowed`, audited, banner-shown via existing company-impersonation path, no new Admin UI; COMPANY_ADMIN: `allowed`, own tenant, no audit; WORKER/PARTNER: `denied`; ADVISOR: `denied` — Assignment-scoped Advisor read is a later, separate KORA-WP-116 concern). Real enforcement is `gov.living_koral_transformation_ledger`/`analytics.living_koral_state`'s own RLS (migration 083, two additive `company_own_..._read` policies mirroring migration 079's precedent) — this row is the app-level mirror, matching this matrix's own existing convention for the other company resources above.
| `company_living_koral_edition` | KORA-WP-115 — KORAL Edition archive (`/company/living-koral/editions`) — own-tenant read (KORA_ADMIN cross-tenant via existing impersonation path, `allowed`/audited; COMPANY_ADMIN own tenant, `allowed`/no audit; WORKER/PARTNER/ADVISOR: `denied`). Create is COMPANY_ADMIN-only, own tenant, exclusively through `analytics.fn_create_living_koral_edition()` (migration 085, hardened `SECURITY DEFINER`, no raw `authenticated` INSERT grant exists) — KORA_ADMIN has no create authority here (pre-check 174 §6, a deliberate constitutive-authority boundary). Real read enforcement is `analytics.living_koral_edition`'s own RLS (migration 085, mirrors migration 083's `company_own_..._read` shape).
